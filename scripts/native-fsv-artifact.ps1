...
param(
    [Parameter(Mandatory)]
    [ValidateSet('Stage', 'Inspect', 'Cleanup', 'Abandon', 'PreAdmissionAbandon', 'Quarantine', 'QuarantineTerminalPartial', 'MigrateLegacy', 'RetireLock', 'RetireTerminalPartialLock', 'RetireInterruptedV2Lock', 'RetireInterruptedCohortLock')]
    [string]$Operation,
...
function Get-AstroFsvProtocolStableProjection {
    ...
    switch ($schema) {
        ...
        'astrolabe.native-fsv-interrupted-cohort-lock-retirement.completion.v1' {
            return [ordered]@{
                schema = $Record.schema; phase = $Record.phase; issue = $Record.issue
                authorization = $Record.authorization; source = $Record.source
                archive = $Record.archive; session = $Record.session
...
function Read-AstroFsvInterruptedCohortOwnerEntries {
    param(
    ...
    if ($ResidentCount -le 0 -or $ProcessCount -ne ($ResidentCount + 1) -or
        $Entries.Count -ne $ProcessCount) {
        Fail-Astro $Code `
            "$Description has an invalid resident/process cardinality" `
            'preserve the cohort generation; require one indexer plus every declared resident'
    }
    ...
    for ($index = 0; $index -lt $Entries.Count; $index++) {
        $entry = $Entries[$index]
        $fields = @($entry.PSObject.Properties | ForEach-Object Name)
        if ($fields.Count -ne 3 -or
            @('role', 'ordinal', 'identity' | Where-Object {
                    $fields -notcontains $_
                }).Count -ne 0) {
            Fail-Astro $Code `
                "$Description entry $index does not have exact role/ordinal/identity fields" `
                'preserve malformed or extended owner state for explicit protocol review'
        }
        $expectedRole = if ($index -eq 0) { 'indexer' } else { 'resident' }
        $expectedOrdinal = if ($index -eq 0) { 0 } else { $index }
        if ([string]$entry.role -cne $expectedRole -or
            [int]$entry.ordinal -ne $expectedOrdinal) {
            Fail-Astro $Code `
                "$Description is not canonically ordered as indexer:0 then resident:1..N" `
                'preserve schedule-dependent or incomplete owner state'
        }
        $identity = Read-AstroFsvProcessIdentity $entry.identity $Code `
            "$Description $expectedRole $expectedOrdinal identity"
        $generationKey = '{0}:{1}' -f $identity.pid,
            $identity.process_start_utc_ticks
        if (-not $generations.Add($generationKey)) {
            Fail-Astro $Code `
                "$Description repeats exact process generation $generationKey" `
                'preserve the ambiguous cohort owner envelope'
        }
...
function Test-AstroFsvInterruptedCohortOwnerEntriesEqual {
    param(
...
function Read-AstroFsvInterruptedCohortLockTrackerEvidence {
    param(
        [Parameter(Mandatory)][string]$Url,
        [Parameter(Mandatory)][int]$ExpectedIssue,
        [Parameter(Mandatory)][string]$ExpectedLockPath,
        ...
        [Parameter(Mandatory)][string]$ExpectedStandardErrorSha256,
        [Parameter(Mandatory)][string]$ExpectedRunRecordPath,
        [Parameter(Mandatory)][string]$ExpectedCohortPlanPath,
        [Parameter(Mandatory)][string]$ExpectedCohortPlanSha256,
        [Parameter(Mandatory)][string]$ExpectedInventorySchema,
        [Parameter(Mandatory)][string]$ExpectedInventoryEncoding,
        [Parameter(Mandatory)][int]$ExpectedInventoryEntryCount,
        [Parameter(Mandatory)][uint64]$ExpectedInventoryCanonicalByteCount,
        ...
        [Parameter(Mandatory)][int]$ExpectedResidentCount,
        [Parameter(Mandatory)][int]$ExpectedProcessCount,
        [Parameter(Mandatory)][string]$ExpectedLauncherJobName,
        [Parameter(Mandatory)][string]$ExpectedRecoveryRecordPath,
        [Parameter(Mandatory)][string]$ExpectedLockArchivePath,
        [Parameter(Mandatory)][string]$ExpectedCompletionRecordPath,
    ...
    $code = 'ASTRO_FSV_INTERRUPTED_COHORT_TRACKER_EVIDENCE'
    $match = [Regex]::Match(
        $Url,
        '^https://github\.com/(?<owner>[^/]+)/(?<repo>[^/]+)/issues/(?<issue>[1-9][0-9]*)#issuecomment-(?<comment>[1-9][0-9]*)\z',
        [Text.RegularExpressions.RegexOptions]::CultureInvariant
    ...
    $prefix = 'ASTRO_FSV_INTERRUPTED_COHORT_LOCK_RETIREMENT_EVIDENCE '
    [string[]]$markers = @([string]$comment.body -split "`r?`n" |
            Where-Object { $_.StartsWith($prefix, [StringComparison]::Ordinal) })
    if ($markers.Count -ne 1) {
        Fail-Astro "${code}_INVALID" `
            "tracker comment must contain exactly one '$prefix' line" `
            'post one fresh machine-readable interrupted-cohort retirement evidence object'
    }
    try { $evidence = $markers[0].Substring($prefix.Length) | ConvertFrom-Json }
    catch {
        Fail-Astro "${code}_INVALID" `
    ...
    $expectedFields = @(
        ...
        'standard_output_path', 'standard_output_sha256', 'standard_error_path',
        'standard_error_sha256', 'run_record_path', 'run_record_state',
        'cohort_plan_path', 'cohort_plan_sha256', 'inventory_schema',
        'inventory_encoding', 'inventory_entry_count',
        'inventory_canonical_byte_count', 'inventory_sha256', 'launcher_identity',
    ...
    foreach ($pair in @(
            ...
            @('standard_error_path', $ExpectedStandardErrorPath),
            @('run_record_path', $ExpectedRunRecordPath),
            @('cohort_plan_path', $ExpectedCohortPlanPath),
            @('recovery_record_path', $ExpectedRecoveryRecordPath),
            @('lock_archive_path', $ExpectedLockArchivePath),
            @('completion_record_path', $ExpectedCompletionRecordPath),
            @('lifecycle_transition_path', $ExpectedLifecycleTransitionPath),
    ...
    $trackerLauncher = Read-AstroFsvProcessIdentity $evidence.launcher_identity `
        "${code}_MISMATCH" 'tracker launcher identity'
    $trackerRunner = Read-AstroFsvProcessIdentity $evidence.runner_identity `
        "${code}_MISMATCH" 'tracker runner identity'
    $trackerProcesses = Read-AstroFsvInterruptedCohortOwnerEntries `
        @($evidence.processes) $ExpectedResidentCount $ExpectedProcessCount `
        "${code}_MISMATCH" 'tracker cohort process set'
    if (-not (Test-AstroFsvIdentityEqual $trackerLauncher $ExpectedLauncherIdentity) -or
        -not (Test-AstroFsvIdentityEqual $trackerRunner $ExpectedRunnerIdentity) -or
        -not (Test-AstroFsvInterruptedCohortOwnerEntriesEqual `
            $trackerProcesses $ExpectedProcesses)) {
        Fail-Astro "${code}_MISMATCH" `
    ...
    if ([string]$evidence.schema -cne
        ...
        [string]$evidence.standard_error_sha256 -cne
            $ExpectedStandardErrorSha256 -or
        [string]$evidence.run_record_state -cne 'absent' -or
        [string]$evidence.cohort_plan_sha256 -cne $ExpectedCohortPlanSha256 -or
        [string]$evidence.inventory_schema -cne $ExpectedInventorySchema -or
        [string]$evidence.inventory_encoding -cne $ExpectedInventoryEncoding -or
        [int]$evidence.inventory_entry_count -ne $ExpectedInventoryEntryCount -or
        [uint64]$evidence.inventory_canonical_byte_count -ne
            $ExpectedInventoryCanonicalByteCount -or
        [string]$evidence.inventory_sha256 -cne $ExpectedInventorySha256 -or
        [int]$evidence.resident_count -ne $ExpectedResidentCount -or
        [int]$evidence.process_count -ne $ExpectedProcessCount -or
        [string]$evidence.launcher_job_name -cne $ExpectedLauncherJobName -or
        [string]$evidence.owner_probe_state -cne 'all-inactive' -or
        [string]$evidence.job_probe_state -cne 'absent' -or
        [string]$evidence.launcher_recovery_completion_sha256 -cne
        ...
        Fail-Astro "${code}_MISMATCH" `
            'tracker evidence differs from the exact local interrupted-cohort binding' `
            're-read physical state and post a fresh exact evidence object'
    }
    return [ordered]@{
        url = $Url
...
function Read-AstroFsvInterruptedCohortPhysicalState {
    param(
        [Parameter(Mandatory)][int]$ExpectedIssue,
        [Parameter(Mandatory)][string]$EvidenceRoot,
        [Parameter(Mandatory)][string]$ReceiptInputPath,
    ...
    $code = 'ASTRO_FSV_INTERRUPTED_COHORT_STATE_INVALID'
    $receiptState = Read-Receipt $ReceiptInputPath $EvidenceRoot
    $inspection = Inspect-ReceiptArtifact $receiptState $EvidenceRoot
    if ([int]$inspection.issue -ne $ExpectedIssue) {
        Fail-Astro $code `
            "requested issue #$ExpectedIssue differs from receipt issue #$($inspection.issue)" `
            'select only the exact interrupted cohort bound to the driving issue'
    }
    $lockFields = @($LockState.PSObject.Properties | ForEach-Object Name)
    $expectedLockFields = @(
        'schema', 'mode', 'issue', 'started', 'resident_count', 'process_count',
        'tree_sha', 'artifact_path', 'artifact_sha256', 'cohort_plan', 'owners',
        'launcher_job', 'phase'
    )
    $lockOwnerFields = if ($LockState.PSObject.Properties['owners']) {
        @($LockState.owners.PSObject.Properties | ForEach-Object Name)
    ...
    $lockPlanFields = if ($LockState.PSObject.Properties['cohort_plan']) {
        @($LockState.cohort_plan.PSObject.Properties | ForEach-Object Name)
    } else { @() }
    [string[]]$liveCapablePhases = @('suspended', 'residents-running', 'indexer-running')
    if ($lockFields.Count -ne $expectedLockFields.Count -or
        @($expectedLockFields | Where-Object { $lockFields -notcontains $_ }).Count -ne 0 -or
        [string]$LockState.schema -cne 'astrolabe.native-fsv-lock.v3' -or
        [string]$LockState.mode -cne 'resident-cohort' -or
        $liveCapablePhases -cnotcontains [string]$LockState.phase -or
        [int]$LockState.issue -ne $ExpectedIssue -or
        [string]$LockState.tree_sha -cne [string]$inspection.tree_sha -or
        -not [string]::Equals(
            [IO.Path]::GetFullPath([string]$LockState.artifact_path),
            $inspection.artifact_path, [StringComparison]::OrdinalIgnoreCase) -or
        [string]$LockState.artifact_sha256 -cne [string]$inspection.sha256 -or
        $lockOwnerFields.Count -ne 3 -or
        @('launcher', 'runner', 'processes' | Where-Object {
                $lockOwnerFields -notcontains $_
            }).Count -ne 0 -or
        $lockJobFields.Count -ne 2 -or
        @('name', 'members' | Where-Object {
        ...
        $lockPlanFields.Count -ne 2 -or
        @('path', 'sha256' | Where-Object {
                $lockPlanFields -notcontains $_
            }).Count -ne 0) {
        Fail-Astro $code `
            'source lock is not one exact live-capable v3 resident-cohort generation' `
            'preserve mixed, pre-live, completed, and non-v3 state'
    }
    $residentCount = [int]$LockState.resident_count
    $processCount = [int]$LockState.process_count
    $lockLauncher = Read-AstroFsvProcessIdentity $LockState.owners.launcher `
        $code 'interrupted cohort lock launcher identity'
    $lockRunner = Read-AstroFsvProcessIdentity $LockState.owners.runner `
        $code 'interrupted cohort lock runner identity'
    $lockProcesses = Read-AstroFsvInterruptedCohortOwnerEntries `
        @($LockState.owners.processes) $residentCount $processCount $code `
        'interrupted cohort lock process set'
    if (-not (Test-AstroFsvIdentityEqual $lockLauncher $inspection.owners.launcher)) {
        Fail-Astro $code `
            'receipt and interrupted cohort lock launcher generations differ' `
            'preserve the cross-generation state and investigate its publisher'
    }
    $matchingLauncherArchive = @($LauncherRecoveryChain.archives | Where-Object {
            [int]$_.authorization.value.generation.launcher_pid -eq
    ...
    if ($matchingLauncherArchive.Count -ne 1 -or
        $jobName -cne [string]$matchingLauncherArchive[0].job_name) {
        Fail-Astro $code `
            'interrupted cohort Job/launcher generation is not the exact archived launcher generation' `
            'supply the exact launcher recovery, target recovery, and pair archive chain'
    }
    [int[]]$jobMembers = @($LockState.launcher_job.members)
    [int[]]$requiredPids = @([int]$lockLauncher.pid, [int]$lockRunner.pid)
    [int[]]$cohortRequiredPids = @(
        @($requiredPids) +
        @($lockProcesses | ForEach-Object { [int]$_.identity.pid }) |
            Sort-Object -Unique
    )
    if ($jobMembers.Count -ne @($jobMembers | Sort-Object -Unique).Count -or
        $jobMembers -contains 0 -or
        @($requiredPids | Where-Object { $jobMembers -notcontains $_ }).Count -ne 0) {
        Fail-Astro $code `
            'interrupted cohort launcher Job membership omits or repeats an exact cohort owner' `
            'preserve the generation and investigate its original attribution'
    }
    ...
    foreach ($claimedPath in @(
        ...
        if (-not $claimedPaths.Add([IO.Path]::GetFullPath($claimedPath))) {
            Fail-Astro $code `
                "interrupted cohort receipt/artifact/live/output/run paths collide: $claimedPath" `
                'preserve the session and bind six pairwise-distinct direct paths'
        }
    }
    if (-not (Test-AstroPathLongPath -LiteralPath $liveStatePath -PathType Leaf) -or
        ...
        Fail-Astro $code `
            'live state and both output files must physically exist' `
            'preserve the session; a complete interrupted cohort publication is not proven'
    }
    if (Test-AstroPathLongPath -LiteralPath $runRecordPath) {
        Fail-Astro $code `
            'expected run record is present' `
    ...
    foreach ($path in @($liveStatePath, $stdoutPath, $stderrPath)) {
        Assert-NotReparseEntry $path 'interrupted cohort session control/output file'
    }
    try { $liveState = Read-AstroUtf8FileLongPath $liveStatePath | ConvertFrom-Json }
    catch {
        Fail-Astro $code `
    ...
    $liveFields = @($liveState.PSObject.Properties | ForEach-Object Name)
    $expectedLiveFields = @(
        'schema', 'mode', 'owners', 'resident_count', 'process_count',
        'launcher_job', 'issue', 'tree_sha', 'artifact', 'cohort_plan',
        'created_at_utc'
    )
    $liveOwnerFields = if ($liveState.PSObject.Properties['owners']) {
        @($liveState.owners.PSObject.Properties | ForEach-Object Name)
    ...
    $livePlanFields = if ($liveState.PSObject.Properties['cohort_plan']) {
        @($liveState.cohort_plan.PSObject.Properties | ForEach-Object Name)
    } else { @() }
    if ($liveFields.Count -ne $expectedLiveFields.Count -or
        @($expectedLiveFields | Where-Object { $liveFields -notcontains $_ }).Count -ne 0 -or
        [string]$liveState.schema -cne 'astrolabe.native-fsv-live.v3' -or
        [string]$liveState.mode -cne 'resident-cohort' -or
        $liveOwnerFields.Count -ne 3 -or
        @('launcher', 'runner', 'processes' | Where-Object {
                $liveOwnerFields -notcontains $_
            }).Count -ne 0 -or
        $liveJobFields.Count -ne 3 -or
        @('name', 'members_before', 'members_with_suspended_cohort' | Where-Object {
                $liveJobFields -notcontains $_
            }).Count -ne 0 -or
        $liveArtifactFields.Count -ne 3 -or
        @('path', 'bytes', 'sha256' | Where-Object {
        ...
        $livePlanFields.Count -ne 2 -or
        @('path', 'sha256' | Where-Object {
                $livePlanFields -notcontains $_
            }).Count -ne 0 -or
        [int]$liveState.issue -ne $ExpectedIssue -or
        [int]$liveState.resident_count -ne $residentCount -or
        [int]$liveState.process_count -ne $processCount -or
        [string]$liveState.tree_sha -cne [string]$inspection.tree_sha -or
        -not [string]::Equals(
            [IO.Path]::GetFullPath([string]$liveState.artifact.path),
            $inspection.artifact_path, [StringComparison]::OrdinalIgnoreCase) -or
        ...
        (@($liveState.launcher_job.members_before) |
            ...
            (@($LockState.launcher_job.members) |
                Sort-Object | ConvertTo-Json -Compress) -or
        -not [string]::Equals(
            [IO.Path]::GetFullPath([string]$liveState.cohort_plan.path),
            [IO.Path]::GetFullPath([string]$LockState.cohort_plan.path),
            [StringComparison]::OrdinalIgnoreCase) -or
        [string]$liveState.cohort_plan.sha256 -cne
            [string]$LockState.cohort_plan.sha256) {
        Fail-Astro $code `
            'live state is not the exact cohort publication bound to the v3 lock' `
            'preserve the cross-generation or drifted state'
    }
    $liveLauncher = Read-AstroFsvProcessIdentity $liveState.owners.launcher `
        $code 'interrupted cohort live launcher identity'
    $liveRunner = Read-AstroFsvProcessIdentity $liveState.owners.runner `
        $code 'interrupted cohort live runner identity'
    $liveProcesses = Read-AstroFsvInterruptedCohortOwnerEntries `
        @($liveState.owners.processes) $residentCount $processCount $code `
        'interrupted cohort live process set'
    if (-not (Test-AstroFsvIdentityEqual $liveLauncher $lockLauncher) -or
        -not (Test-AstroFsvIdentityEqual $liveRunner $lockRunner) -or
        -not (Test-AstroFsvInterruptedCohortOwnerEntriesEqual `
            $liveProcesses $lockProcesses)) {
        Fail-Astro $code `
            'v3 live-state owner generations differ from the source lock' `
            'preserve the session and investigate cross-run provenance'
    }
    [int[]]$liveJobMembers = @($liveState.launcher_job.members_with_suspended_cohort)
    if ($liveJobMembers.Count -ne @($liveJobMembers | Sort-Object -Unique).Count -or
        @($cohortRequiredPids | Where-Object {
                $liveJobMembers -notcontains $_
            }).Count -ne 0) {
        Fail-Astro $code `
            'v3 live-state Job membership omits or repeats an exact cohort owner' `
            'preserve the inconsistent owner publication'
    }
    $cohortPlanPath = [IO.Path]::GetFullPath([string]$LockState.cohort_plan.path)
    if (-not (Test-AstroPathLongPath -LiteralPath $cohortPlanPath -PathType Leaf)) {
        Fail-Astro $code `
            "bound cohort plan is absent: $cohortPlanPath" `
            'preserve the generation until its immutable plan can be read'
    }
    Assert-NotReparseEntry $cohortPlanPath 'interrupted cohort plan'
    $cohortPlanSha256 = File-Sha256 $cohortPlanPath
    if ($cohortPlanSha256 -cne [string]$LockState.cohort_plan.sha256) {
        Fail-Astro $code `
            'bound cohort plan bytes changed after live publication' `
            'preserve the cohort generation and investigate plan drift'
    }
    ...
    $inventoryPaths = @($sessionFiles | ForEach-Object {
    ...
    if ($sessionFiles.Count -ne 5 -or
        ...
        @($allowedPaths | Where-Object { $inventoryPaths -notcontains $_ }).Count -ne 0) {
        Fail-Astro $code `
            'interrupted cohort session inventory is not exactly artifact/receipt/live/stdout/stderr' `
            'preserve any missing, unexpected, nested, or mixed-generation state'
    }
    ...
    foreach ($binding in @(
            New-AstroFsvOwnerBinding 'launcher' 'artifact receipt' $inspection.owners.launcher
            New-AstroFsvOwnerBinding 'promoter' 'artifact receipt' $inspection.owners.promoter
            New-AstroFsvOwnerBinding 'launcher' 'interrupted cohort lock' $lockLauncher
            New-AstroFsvOwnerBinding 'runner' 'interrupted cohort lock' $lockRunner
            New-AstroFsvOwnerBinding 'launcher' 'interrupted cohort live state' $liveLauncher
            New-AstroFsvOwnerBinding 'runner' 'interrupted cohort live state' $liveRunner
        )) { $ownerBindingList.Add($binding) }
    foreach ($entry in $lockProcesses) {
        $ownerBindingList.Add((New-AstroFsvOwnerBinding `
                    ([string]$entry.role) `
                    "interrupted cohort lock ordinal $($entry.ordinal)" `
                    $entry.identity))
    }
    foreach ($entry in $liveProcesses) {
        $ownerBindingList.Add((New-AstroFsvOwnerBinding `
                    ([string]$entry.role) `
                    "interrupted cohort live ordinal $($entry.ordinal)" `
                    $entry.identity))
    }
    foreach ($archive in $LauncherRecoveryChain.archives) {
        $generation = $archive.authorization.value.generation
    ...
    return [ordered]@{
        ...
        lock_launcher = $lockLauncher
        lock_runner = $lockRunner
        lock_processes = $lockProcesses
        resident_count = $residentCount
        process_count = $processCount
        launcher_job_name = $jobName
        cohort_plan_path = $cohortPlanPath
        cohort_plan_sha256 = $cohortPlanSha256
        live_state_path = $liveStatePath
        live_state = $liveState
        live_state_sha256 = File-Sha256 $liveStatePath
        stdout_path = $stdoutPath
...
function Read-AstroFsvInterruptedCohortTrackerForState {
    param(
    ...
    return Read-AstroFsvInterruptedCohortLockTrackerEvidence `
        -Url $TrackerUrl -ExpectedIssue $ExpectedIssue `
        -ExpectedLockPath $FsvLockPath `
        -ExpectedLockSha256 $State.lock_sha256 `
        -ExpectedReceiptPath $State.receipt_state.Path `
        -ExpectedReceiptSha256 $State.receipt_sha256 `
        -ExpectedArtifactPath $State.inspection.artifact_path `
        -ExpectedArtifactSha256 $State.inspection.sha256 `
        ...
        -ExpectedStandardErrorSha256 $State.stderr_sha256 `
        -ExpectedRunRecordPath $State.run_record_path `
        -ExpectedCohortPlanPath $State.cohort_plan_path `
        -ExpectedCohortPlanSha256 $State.cohort_plan_sha256 `
        -ExpectedInventorySchema $State.session_tree.schema `
        -ExpectedInventoryEncoding $State.session_tree.encoding `
        -ExpectedInventoryEntryCount ([int]$State.session_tree.entry_count) `
        -ExpectedInventoryCanonicalByteCount `
        ...
        -ExpectedLauncherIdentity $State.lock_launcher `
        -ExpectedRunnerIdentity $State.lock_runner `
        -ExpectedProcesses $State.lock_processes `
        -ExpectedResidentCount $State.resident_count `
        -ExpectedProcessCount $State.process_count `
        -ExpectedLauncherJobName $State.launcher_job_name `
        -ExpectedRecoveryRecordPath $AuthorizationPath `
...
function Invoke-AstroFsvInterruptedCohortLockRetirementResume {
    param(
        [Parameter(Mandatory)][int]$ExpectedIssue,
        [Parameter(Mandatory)][string]$Workspace,
        [Parameter(Mandatory)][string]$EvidenceRoot,
    ...
    $authorizationPath = Assert-PathWithin $RecoveryInputPath $RecoveryRoot $code `
        'interrupted-cohort retirement authorization path'
    $archivePath = $authorizationPath + '.lock.bin'
    $completionPath = $authorizationPath + '.completed.json'
    $transitionPresent = (Get-AstroFsvStrictPathState `
            $LifecycleTransitionPath file $code 'canonical FSV lifecycle transition').state -ceq
        'present'
    $authorizationPresent = (Get-AstroFsvStrictPathState `
            $authorizationPath file $code 'interrupted-cohort authorization archive').state -ceq
        'present'
    if ($transitionPresent -and $authorizationPresent) {
        Fail-Astro $code 'canonical transition and authorization archive are both present' `
            'preserve both namespaces and investigate the interrupted no-replace archive'
    ...
    if ([string]$authorization.schema -cne
        ...
        [string]$authorization.failure.message -cne $FailureMessage) {
        Fail-Astro $code `
            'surviving authorization is not the requested interrupted-cohort transaction' `
            'preserve it and resume only with its exact issue/tracker/reason binding'
    }
    foreach ($pair in @(
            @([string]$authorization.authorization_record_path, $authorizationPath),
        ...
        if (-not [string]::Equals(
                ...
                [StringComparison]::OrdinalIgnoreCase)) {
            Fail-Astro $code `
                'surviving authorization path binding differs from this request' `
                'preserve the transaction and pass its exact original paths'
        }
    }
    $launcherChain = Read-AstroFsvLauncherRecoveryChain `
    ...
    if ($launcherProtocol.State -ne 'absent' -or
        (Test-AstroPathLongPath -LiteralPath $targetPath)) {
        Fail-Astro $code `
            "launcher protocol='$($launcherProtocol.State)'; target_present=$(Test-AstroPathLongPath -LiteralPath $targetPath) during interrupted-cohort resume" `
            'complete the exact launcher terminal lifecycle before resuming FSV recovery'
    }
    $state = Read-AstroFsvInterruptedCohortPhysicalState `
        -ExpectedIssue $ExpectedIssue -EvidenceRoot $EvidenceRoot `
        -ReceiptInputPath $ReceiptInputPath -LockState $authorization.fsv_lock.state `
        -LockSha256 ([string]$authorization.fsv_lock.sha256) `
        -LiveStateInputPath $LiveStateInputPath `
    ...
    if ([string]$authorization.receipt_sha256 -cne [string]$state.receipt_sha256 -or
        [string]$authorization.artifact.sha256 -cne
            [string]$state.inspection.sha256 -or
        [string]$authorization.cohort_plan.sha256 -cne
            [string]$state.cohort_plan_sha256 -or
        [string]$authorization.live_state.sha256 -cne
            [string]$state.live_state_sha256 -or
        [string]$authorization.outputs.stdout.sha256 -cne
            [string]$state.stdout_sha256 -or
        ...
        [string]$authorization.launcher_job.name -cne
            [string]$state.launcher_job_name) {
        Fail-Astro $code `
            'session bytes differ from the durable interrupted-cohort authorization' `
            'preserve every byte and investigate post-authorization drift'
    }
    Assert-AstroFsvPersistedOwnerEnvelope `
        -Owners $authorization.owners -ExpectedBindings $state.owner_bindings `
        -Code $code -Description 'persisted interrupted-cohort owner envelope'
    Assert-AstroFsvPersistedAbsentJobProbe `
        $authorization.launcher_job.initial_probe $state.launcher_job_name $code `
        'persisted interrupted-cohort initial Job probe'
    Assert-AstroFsvPersistedAbsentJobProbe `
        $authorization.launcher_job.final_probe $state.launcher_job_name $code `
    ...
    $initialOwnerProbes = @(Assert-AstroFsvOwnersInactive `
            -Bindings $state.owner_bindings `
            -CodePrefix 'ASTRO_FSV_INTERRUPTED_COHORT_RESUME' `
            -Description 'interrupted-cohort retirement resume')
    $jobProbeFirst = Get-AstroLauncherJobObjectProbe -Name $state.launcher_job_name
    if ($jobProbeFirst.State -cne 'absent') {
        Fail-Astro $code `
            "interrupted-cohort launcher Job is '$($jobProbeFirst.State)' during resume" `
            'preserve every byte until the exact deterministic Job is absent'
    }
    $sourcePresent = (Get-AstroFsvStrictPathState `
            $FsvLockPath file $code 'interrupted cohort FSV lock source').state -ceq
        'present'
    $archivePresent = (Get-AstroFsvStrictPathState `
            $archivePath file $code 'interrupted cohort FSV lock archive').state -ceq
        'present'
    if ($sourcePresent -eq $archivePresent) {
        Fail-Astro $code `
            'exactly one of interrupted-cohort lock source/archive must exist before completion' `
            'preserve ambiguous namespace state and investigate the publication boundary'
    }
    if ($sourcePresent) {
        if ((File-Sha256 $FsvLockPath) -cne
    ...
    $finalOwnerProbes = @(Assert-AstroFsvOwnersInactive `
            -Bindings $state.owner_bindings `
            -CodePrefix 'ASTRO_FSV_INTERRUPTED_COHORT_RESUME' `
            -Description 'interrupted-cohort retirement resume final authorization')
    $jobProbeSecond = Get-AstroLauncherJobObjectProbe -Name $state.launcher_job_name
    if ($jobProbeSecond.State -cne 'absent') {
        Fail-Astro $code `
            "interrupted-cohort launcher Job became '$($jobProbeSecond.State)'" `
            'preserve authorization/archive state and investigate generation reappearance'
    }
    $completionState = Get-AstroFsvStrictPathState `
        $completionPath file $code 'interrupted-cohort retirement completion'
    if ($completionState.state -ceq 'absent') {
        $completion = [ordered]@{
            schema = 'astrolabe.native-fsv-interrupted-cohort-lock-retirement.completion.v1'
            phase = 'complete-lock-archived-source-absent'
            issue = $ExpectedIssue
            completed_at_utc = [DateTime]::UtcNow.ToString('o')
            authorization = [ordered]@{
        ...
        $completionRecord = Publish-NewAstroFsvProtocolRecord `
            -Path $completionPath -Value $completion `
            -CodePrefix 'ASTRO_FSV_INTERRUPTED_COHORT_COMPLETION'
    }
    else {
        $completionRecord = Read-AstroFsvExactJsonFile `
            $completionPath $code 'interrupted-cohort retirement completion'
    }
    $completionValue = $completionRecord.value
    if ([string]$completionValue.schema -cne
            'astrolabe.native-fsv-interrupted-cohort-lock-retirement.completion.v1' -or
        [string]$completionValue.phase -cne 'complete-lock-archived-source-absent' -or
        [int]$completionValue.issue -ne $ExpectedIssue -or
        [string]$completionValue.authorization.sha256 -cne
            [string]$authorizationRecord.sha256 -or
        ...
        [string]$completionValue.session.inventory_sha256 -cne
            [string]$state.session_tree.sha256) {
        Fail-Astro $code `
            'completion does not hash-link source absence and unchanged cohort session' `
            'preserve the complete record chain and investigate malformed completion bytes'
    }
    if ($transitionPresent) {
        Move-AstroFileWriteThroughNoReplace `
    ...
    if ((Test-AstroPathLongPath -LiteralPath $LifecycleTransitionPath) -or
        ...
        (File-Sha256 $archivePath) -cne [string]$authorization.fsv_lock.sha256 -or
        (File-Sha256 $state.live_state_path) -cne [string]$state.live_state_sha256 -or
        (File-Sha256 $state.cohort_plan_path) -cne
            [string]$state.cohort_plan_sha256 -or
        (Get-AstroOrdinaryDirectoryTreeInventoryLongPath `
            $state.session_directory).sha256 -cne [string]$state.session_tree.sha256) {
        Fail-Astro $code 'terminal readback differs from interrupted-cohort completion' `
            'preserve every recovery record and inspect physical state'
    }
    return [ordered]@{
        operation = 'retire-interrupted-cohort-lock'
        authorization_path = $authorizationPath
        authorization_sha256 = File-Sha256 $authorizationPath
        completion_path = $completionPath
        completion_sha256 = File-Sha256 $completionPath
...
try {
    switch ($Operation) {
        ...
        'RetireInterruptedCohortLock' {
            if ($Issue -le 0) {
                Fail-Astro 'ASTRO_FSV_INTERRUPTED_COHORT_ISSUE_INVALID' `
                    'Issue must be positive for interrupted-cohort lock retirement' `
                    'pass the exact issue bound by the staged artifact and FSV lock'
            }
            if ([string]::IsNullOrWhiteSpace($TrackerCommentUrl)) {
                Fail-Astro 'ASTRO_FSV_INTERRUPTED_COHORT_TRACKER_REQUIRED' `
                    'TrackerCommentUrl is required for interrupted-cohort lock retirement' `
            ...
            if ($ReasonCode -notmatch '^[A-Z][A-Z0-9_]{2,95}$' -or
                ...
                Fail-Astro 'ASTRO_FSV_INTERRUPTED_COHORT_REASON_INVALID' `
                    'RetireInterruptedCohortLock requires a structured code and nonblank message' `
            ...
            if (-not $fsvLifecycleMutex.Acquired) {
                ...
                Fail-Astro 'ASTRO_FSV_LIFECYCLE_MUTEX_HELD' `
                    ...
                    'wait for the exact active lifecycle transaction to finish'
            }
            ...
            if (-not $launcherCoordinationMutex.Acquired) {
                ...
                Exit-AstroFsvLifecycleMutex $fsvLifecycleMutex
                Fail-Astro 'ASTRO_FSV_INTERRUPTED_COHORT_LAUNCHER_MUTEX_HELD' `
                    "launcher coordination mutex is held: $($launcherCoordinationMutex.Name)" `
            ...
            try {
                $authorizationPath = Assert-PathWithin `
                    ...
                    'ASTRO_FSV_INTERRUPTED_COHORT_RECORD_ESCAPE' `
                    'interrupted-cohort retirement authorization path'
                ...
                if ((Test-AstroPathLongPath -LiteralPath $fsvLifecycleTransition) -or
                    ...
                    $resumeResult = Invoke-AstroFsvInterruptedCohortLockRetirementResume `
                ...
                if ($launcherProtocol.State -ne 'absent' -or
                    (Test-AstroPathLongPath -LiteralPath $targetPath)) {
                    Fail-Astro 'ASTRO_FSV_INTERRUPTED_COHORT_LAUNCHER_STATE' `
                        "launcher protocol='$($launcherProtocol.State)'; target_present=$(Test-AstroPathLongPath -LiteralPath $targetPath)" `
                        'complete the exact launcher and preserved-target lifecycle first'
                }
                if (-not (Test-AstroPathLongPath -LiteralPath $fsvLock -PathType Leaf)) {
                    Fail-Astro 'ASTRO_FSV_INTERRUPTED_COHORT_LOCK_ABSENT' `
                        "FSV lock is already absent: $fsvLock" `
                        'do not manufacture interrupted-cohort evidence for an absent lock'
                }
                Assert-NotReparseEntry $fsvLock 'interrupted cohort native-FSV lock'
                $lockSha256 = File-Sha256 $fsvLock
                try { $lockState = Read-AstroUtf8FileLongPath $fsvLock | ConvertFrom-Json }
                catch {
                    Fail-Astro 'ASTRO_FSV_INTERRUPTED_COHORT_LOCK_INVALID' `
                ...
                $initialOwnerProbes = @(Assert-AstroFsvOwnersInactive `
                        -Bindings $state.owner_bindings `
                        -CodePrefix 'ASTRO_FSV_INTERRUPTED_COHORT' `
                        -Description 'interrupted-cohort lock retirement')
                $jobProbeFirst = Get-AstroLauncherJobObjectProbe `
                    -Name $state.launcher_job_name
                if ($jobProbeFirst.State -cne 'absent') {
                ...
                $tracker = Read-AstroFsvInterruptedCohortTrackerForState `
                    -State $state -TrackerUrl $TrackerCommentUrl `
                    -ExpectedIssue $Issue -FsvLockPath $fsvLock `
                ...
                $stateFinal = Read-AstroFsvInterruptedCohortPhysicalState `
                    -ExpectedIssue $Issue -EvidenceRoot $evidenceRoot `
                    -ReceiptInputPath $ReceiptPath -LockState $lockState `
                    -LockSha256 $lockSha256 -LiveStateInputPath $LiveStatePath `
                    -StandardOutputInputPath $StandardOutputPath `
                ...
                if ([string]$stateFinal.session_tree.sha256 -cne
                        [string]$state.session_tree.sha256 -or
                    [string]$stateFinal.cohort_plan_sha256 -cne
                        [string]$state.cohort_plan_sha256) {
                    Fail-Astro 'ASTRO_FSV_INTERRUPTED_COHORT_STATE_DRIFT' `
                        'session inventory or cohort plan changed before final authorization' `
                        'preserve every byte and investigate the competing writer'
                }
                $finalOwnerProbes = @(Assert-AstroFsvOwnersInactive `
                        -Bindings $stateFinal.owner_bindings `
                        -CodePrefix 'ASTRO_FSV_INTERRUPTED_COHORT' `
                        -Description 'interrupted-cohort final authorization')
                $jobProbeSecond = Get-AstroLauncherJobObjectProbe `
                ...
                $authorization = [ordered]@{
                    ...
                    cohort_plan = [ordered]@{
                        path = $state.cohort_plan_path
                        sha256 = $state.cohort_plan_sha256
                    }
                    ...
                    launcher_recovery_chain = [ordered]@{
                        ...
                        launcher_archive_completions = @(
                            foreach ($archivePath in $LauncherArchiveCompletionPaths) {
                                $fullArchivePath = [IO.Path]::GetFullPath($archivePath)
                                [ordered]@{
                                    path = $fullArchivePath
                                    sha256 = File-Sha256 $fullArchivePath
                                }
                            }
                        )
                    }
                ...
                $publication = Publish-NewAstroFsvProtocolRecord `
                    ...
                    -CodePrefix 'ASTRO_FSV_INTERRUPTED_COHORT_AUTHORIZATION' `
                ...
                if ([string]$publication.value.fsv_lock.sha256 -cne $lockSha256 -or
                    [string]$publication.value.session_inventory.sha256 -cne
                        [string]$state.session_tree.sha256 -or
                    [string]$publication.value.cohort_plan.sha256 -cne
                        [string]$state.cohort_plan_sha256) {
                    Fail-Astro 'ASTRO_FSV_INTERRUPTED_COHORT_RECORD_INVALID' `
                ...
                $result = Invoke-AstroFsvInterruptedCohortLockRetirementResume `
                    -ExpectedIssue $Issue -Workspace $workspace `
                    -EvidenceRoot $evidenceRoot -RecoveryRoot $recoveryRoot `
                    -FsvLockPath $fsvLock `
                    -LifecycleTransitionPath $fsvLifecycleTransition `
