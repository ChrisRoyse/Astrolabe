---
name: pushrepo
description: Commits and pushes all repository changes with ASTROLABE discipline — issue-referenced commit body, hygiene sweep (no target/, .tmp, scratch, logs, or test databases staged), secret-scanner awareness, and pushed-state readback. Replaces the former pushrepo command, which invoked a validation hook that does not exist in this repository.
