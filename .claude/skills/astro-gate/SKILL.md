...
- **Long runs (#197 rule 4):** a cold aggregate is a multi-hour/15-min+ run and must not live inside a foreground tool call that can time out. Launch it detached, post the launcher PID to the driving issue at start, and let other sessions verify liveness instead of guessing.
