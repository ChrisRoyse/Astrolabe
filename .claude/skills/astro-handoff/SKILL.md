...
- Confirm `target/` is absent (`ls target` must fail). There is no launcher lock, lease, or ownership protocol — that machinery is retired. A single operator on one machine means the process that created `target/` simply deletes it.
