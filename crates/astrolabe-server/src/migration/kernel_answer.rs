...
const GET_KERNEL_MODES: [&str; 3] = ["read", "build", "gaps"];
...
pub(crate) fn handle_get_kernel(args_json: &str) -> Result<String, DynError> {
    ...
    if !GET_KERNEL_MODES.contains(&mode) {
        return tool_error_result(format!(
            "{ASTRO_GET_KERNEL_MODE_UNSUPPORTED}: get_kernel mode {mode:?} is not available; remediation: use mode=\"read\", mode=\"gaps\", or mode=\"build\""
        ));
    }
