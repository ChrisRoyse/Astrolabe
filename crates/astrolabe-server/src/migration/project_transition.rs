...
fn normalization_receipt_matches(receipt: &Value) -> bool {
    ...
    // Holder-inventory stability is proven by the Windows Restart Manager probe.
    // On a host without that API the probe never runs, so `holder_inventory_stable`
    // stays false forever and this precondition becomes unsatisfiable rather than
    // protective -- every supervised index refuses with
    // ASTRO_PROJECT_TRANSITION_WORKER_RECEIPT_MISMATCH.
    //
    // With no probing API there is no holder inventory that could be unstable, so
    // absence is treated as vacuously clear. This keys off the receipt's own
    // recorded probe status rather than a compile-time platform assumption, so a
    // host that CAN probe is still held to the real stability requirement, and a
    // probe that ran and found holders still fails.
    //
    // CBM_PROJECT_HOLDER_PROBE_NOT_RUN = 0, _API_UNAVAILABLE = 2.
    let holder_count_zero = normalization
        .pointer("/post_close_quiescence/holder_count")
        .and_then(Value::as_u64)
        == Some(0);
    let probe_status = normalization
        .pointer("/post_close_quiescence/holder_probe_status")
        .and_then(Value::as_i64);
    let probe_unavailable = matches!(probe_status, Some(0) | Some(2));
    let holder_clear = holder_count_zero
        && (normalization
