...
fn installer_platform() -> InstallerPlatform {
    if cfg!(windows) {
...
fn configured_path(value: Option<PathBuf>, fallback: PathBuf) -> PathBuf {
    value
        .filter(|path| !path.as_os_str().is_empty())
        .unwrap_or(fallback)
}

fn installer_config_dir_for(
...
fn installer_local_dir(home: &Path) -> PathBuf {
...
fn known_installer_config_files_for_dirs(home: &Path, config: &Path, local: &Path) -> Vec<PathBuf> {
    vec![
