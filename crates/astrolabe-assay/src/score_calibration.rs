...
mod tests {
    ...
    #[test]
    fn every_calibration_knob_declares_bounds_that_contain_its_default() {
        assert!(!ASSAY_CALIBRATION_KNOBS.is_empty());
        for knob in ASSAY_CALIBRATION_KNOBS {
            assert_eq!(
                knob.registry_version, ASSAY_CALIBRATION_KNOB_REGISTRY_VERSION,
                "{knob:?}"
            );
    ...
    #[test]
    fn calibrates_measured_thresholds_from_the_distribution() {
        // Eight conforming scores at 0 and one planted outlier at 800 millipoints.
        let scores = [0, 0, 0, 0, 0, 0, 0, 0, 800];
        let cal = calibrate_score_distribution(
            "SIM_STRUCTxSIM_SEMANTIC",
            &scores,
            &CalibrationConfig::default(),
        )
        .expect("calibrates");
        assert_eq!(cal.sample_size, 9);
        assert_eq!(cal.n_eff, 9);
        // mean = 800/9 = 88.9 -> 89; population std = sqrt(568888.9/9) = 251.4 -> 251.
        assert_eq!(cal.mean_millipoints, 89);
        assert_eq!(cal.std_dev_millipoints, 251);
        // medium = round(88.9 + 1.0*251.4) = 340; high = round(88.9 + 2.0*251.4) = 592.
        assert_eq!(cal.medium_min_score_millipoints, 340);
        assert_eq!(cal.high_min_score_millipoints, 592);
        // The planted outlier (800) clears the high tier; the conforming 0s clear
        // neither tier.
        assert!(800 >= cal.high_min_score_millipoints);
        assert!(0 < cal.medium_min_score_millipoints);
    }

    #[test]
    fn different_spreads_yield_different_thresholds() {
        let tight = [100, 100, 100, 100, 110, 110, 90, 90, 200];
        let wide = [0, 0, 0, 0, 0, 0, 0, 0, 900];
        let a = calibrate_score_distribution("pair", &tight, &CalibrationConfig::default())
            .expect("tight");
        let b = calibrate_score_distribution("pair", &wide, &CalibrationConfig::default())
            .expect("wide");
        assert_ne!(a.std_dev_millipoints, b.std_dev_millipoints);
        assert_ne!(
            a.high_min_score_millipoints, b.high_min_score_millipoints,
            "distinct spreads must produce distinct high thresholds"
        );
    }

    #[test]
    fn below_floor_refuses_with_deficit() {
        let scores = [0, 800, 0];
        let err = calibrate_score_distribution("pair", &scores, &CalibrationConfig::default())
            .expect_err("below floor refuses");
        assert_eq!(err.code(), ASTRO_ASSAY_CALIBRATION_BELOW_FLOOR);
        assert!(err.message().contains("below the floor"));
    }

    #[test]
    fn zero_spread_refuses_as_degenerate() {
        let scores = [300; 12];
        let err = calibrate_score_distribution("pair", &scores, &CalibrationConfig::default())
            .expect_err("degenerate refuses");
        assert_eq!(err.code(), ASTRO_ASSAY_CALIBRATION_DEGENERATE);
    }

    #[test]
    fn out_of_range_score_is_input_invalid() {
        let scores = [0, 0, 0, 0, 0, 0, 0, 0, 1_001];
        let err = calibrate_score_distribution("pair", &scores, &CalibrationConfig::default())
            .expect_err("out of range refused");
        assert_eq!(err.code(), ASTRO_ASSAY_CALIBRATION_INPUT_INVALID);
    }

    #[test]
    fn calibration_is_deterministic_and_round_trips_through_bytes() {
        let scores = [0, 10, 20, 30, 40, 50, 60, 70, 900];
        let first = calibrate_score_distribution("pair", &scores, &CalibrationConfig::default())
            .expect("first");
        let second = calibrate_score_distribution("pair", &scores, &CalibrationConfig::default())
            .expect("second");
        assert_eq!(first, second);
        let bytes = calibration_dump_bytes(&first);
        let readback = read_calibration_bytes(&bytes).expect("round trip");
        assert_eq!(readback, first);
        assert_eq!(calibration_dump_bytes(&readback), bytes);
    }
}
