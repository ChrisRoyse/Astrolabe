...
mod tests {
    ...
    fn stream_is_a_pure_function_of_seed_and_index() {
        let mut a = DeterministicRng::from_u64(7);
        let mut b = DeterministicRng::from_u64(7);
        for _ in 0..64 {
            assert_eq!(a.next_u64(), b.next_u64());
        }
        // A different seed gives a different stream.
        let mut c = DeterministicRng::from_u64(8);
        let mut d = DeterministicRng::from_u64(7);
        assert_ne!(c.next_u64(), d.next_u64());
    }

    #[test]
    fn labeled_streams_are_independent() {
        let mut boot = DeterministicRng::from_u64_labeled(1, "bootstrap");
        let mut post = DeterministicRng::from_u64_labeled(1, "posterior");
        assert_ne!(boot.next_u64(), post.next_u64());
    }
    ...
    #[test]
    fn standard_normal_has_near_zero_mean_and_unit_variance() {
    ...
    fn gamma_mean_matches_shape() {
        ...
        for shape in [0.5_f64, 1.0, 2.5, 7.0] {
            let mut rng = DeterministicRng::from_u64_labeled(99, "gamma");
            let n = 200_000;
            let mut sum = 0.0;
            for _ in 0..n {
                sum += rng.next_gamma(shape);
            }
            let mean = sum / n as f64;
            assert!(
    ...
    fn dirichlet_sums_to_one() {
        ...
        for _ in 0..1000 {
            let p = rng.next_dirichlet(&[0.5, 0.5, 0.5, 0.5]);
