...
mod tests {
    ...
    fn cfg() -> CalibrationConfig {
        CalibrationConfig::from_defaults().unwrap()
    }

    #[test]
    fn measured_precision_supersedes_a_contradicted_prior() {
        // service_pattern's CBM prior is 0.5, but in this repo 8 of 40 confirmed —
        // the prior badly overstates it. Above quorum the measured 0.2 replaces it.
        let obs = vec![StrategyObservation {
        ...
        let s = &card.strategies[0];
        assert_eq!(s.source, CalibrationSource::Measured);
        assert_eq!(s.trust, TrustTag::Trusted);
        let m = s.measured.as_ref().unwrap();
        assert!((m.precision - 0.2).abs() < 1e-12);
        assert_eq!(m.n, 40);
        // The CI brackets the measured point and stays in [0,1].
        assert!(m.ci_lo > 0.0 && m.ci_lo < 0.2, "ci_lo={}", m.ci_lo);
        assert!(m.ci_hi > 0.2 && m.ci_hi < 0.5, "ci_hi={}", m.ci_hi);
        // The effective confidence is the measurement, not the (wrong) prior.
        assert!((s.effective_confidence - 0.2).abs() < 1e-12);
    }

    #[test]
    fn below_quorum_retains_the_prior_as_fallback() {
        let obs = vec![StrategyObservation {
        ...
        let card = build_calibration_card(&obs, &cfg()).unwrap();
        let s = &card.strategies[0];
        assert_eq!(s.source, CalibrationSource::PriorFallback);
        assert_eq!(s.trust, TrustTag::Provisional);
        assert!(s.measured.is_none());
        assert!((s.effective_confidence - 0.55).abs() < 1e-12);
    }
