...
mod tests {
    ...
    #[test]
    fn planted_seven_day_cadence_is_recovered() {
        // Daily samples of a 7-day sinusoid → peak period ≈ 7, FAP ≈ 0.
        ...
        let values: Vec<f64> = times
            ...
            .collect();
        let card = measure_periodicity("failures", &times, &values, 1, &cfg).unwrap();
        assert!(
            (card.peak_period - 7.0).abs() < 0.3,
            "peak_period={}",
            card.peak_period
        );
        assert!(card.detected, "should detect (fap={})", card.fap_permille);
        assert!(card.fap_permille <= 50, "fap={}", card.fap_permille);
        // Autocorrelation corroborates at lag 7.
        assert_eq!(
            card.autocorr_peak_lag, 7,
            "autocorr lag={}",
            card.autocorr_peak_lag
        );
    }

    #[test]
    fn white_noise_is_not_flagged() {
        // A noise series has no dominant period: FAP high, not detected.
        let cfg = DiffConfig::from_defaults().unwrap();
        let n = 210;
        let mut rng = DeterministicRng::from_u64_labeled(99, "noise");
        let times: Vec<f64> = (0..n).map(|i| i as f64).collect();
        let values: Vec<f64> = (0..n).map(|_| rng.next_standard_normal()).collect();
        let card = measure_periodicity("noise", &times, &values, 2, &cfg).unwrap();
        assert!(
            !card.detected,
            "should not detect (fap={})",
            card.fap_permille
        );
        assert!(card.fap_permille > 50, "fap={}", card.fap_permille);
    }
    ...
    #[test]
    fn constant_series_is_not_flagged() {
        let cfg = DiffConfig::from_defaults().unwrap();
        let times: Vec<f64> = (0..60).map(|i| i as f64).collect();
        let values = vec![3.0_f64; 60];
        let card = measure_periodicity("flat", &times, &values, 1, &cfg).unwrap();
        assert!(!card.detected);
        assert_eq!(card.peak_power, 0.0);
        assert_eq!(card.fap_permille, 1000);
    }
    ...
    fn deterministic_under_fixed_seed() {
        ...
        let values: Vec<f64> = times
            ...
            .collect();
        let a = measure_periodicity("s", &times, &values, 7, &cfg).unwrap();
        let b = measure_periodicity("s", &times, &values, 7, &cfg).unwrap();
        assert_eq!(a, b);
    }
    ...
    fn too_short_series_is_rejected() {
        let cfg = DiffConfig::from_defaults().unwrap();
        let err = measure_periodicity("s", &[0.0, 1.0], &[1.0, 2.0], 1, &cfg).unwrap_err();
    ...
    fn non_increasing_time_axis_is_rejected() {
        ...
        let err = measure_periodicity("s", &times, &values, 1, &cfg).unwrap_err();
        assert_eq!(err.code(), ASTRO_ASSAY_MEASUREMENT_INPUT_INVALID);
    }
}
