...
mod tests {
    ...
    fn normal_quantile_matches_known_values() {
        // Φ⁻¹(0.5) = 0; Φ⁻¹(0.975) = 1.959964; Φ⁻¹(0.95) = 1.644854.
        assert!(inverse_standard_normal_cdf(0.5).abs() < 1e-9);
        assert!((inverse_standard_normal_cdf(0.975) - 1.959_963_985).abs() < 1e-6);
        assert!((inverse_standard_normal_cdf(0.95) - 1.644_853_627).abs() < 1e-6);
        // Symmetry: Φ⁻¹(p) = -Φ⁻¹(1-p).
        assert!((inverse_standard_normal_cdf(0.1) + inverse_standard_normal_cdf(0.9)).abs() < 1e-9);
    }

    #[test]
    fn wilson_brackets_point_and_stays_in_unit_range() {
        // 8/10 at 95%: point 0.8 lies inside, and the interval stays in [0,1].
        let (lo, hi) = wilson_interval(8, 10, 0.95);
        assert!(lo > 0.0 && lo < 0.8, "lo={lo}");
        assert!(hi > 0.8 && hi <= 1.0, "hi={hi}");
        // A perfect 10/10 does not exceed 1.0 (the Wald interval would collapse to a point).
        let (plo, phi) = wilson_interval(10, 10, 0.95);
        assert!(phi <= 1.0 && plo < 1.0, "plo={plo} phi={phi}");
        assert!(
            plo > 0.0,
            "Wilson keeps a lower bound off zero at 10/10: {plo}"
        );
    }

    #[test]
    fn wilson_narrows_as_trials_grow() {
        let (lo_small, hi_small) = wilson_interval(80, 100, 0.95);
        let (lo_big, hi_big) = wilson_interval(800, 1000, 0.95);
        assert!(
            (hi_big - lo_big) < (hi_small - lo_small),
            "more trials must tighten the interval"
        );
    }
}
