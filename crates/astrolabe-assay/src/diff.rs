...
mod tests {
    ...
    #[test]
    fn defaults_resolve_and_te_sweep_is_dyadic() {
        let cfg = DiffConfig::from_defaults().unwrap();
        assert_eq!(cfg.diff_bins, 4);
        assert_eq!(cfg.redundancy_quorum, 50);
        assert_eq!(cfg.synergy_quorum, 150);
        assert!((cfg.redundancy_retire_nmi - 0.6).abs() < 1e-12);
        assert_eq!(cfg.te_lags(), vec![1, 2, 4, 8]);
    }

    #[test]
    fn discretize_scalar_and_label() {
        let cfg = DiffConfig::from_defaults().unwrap();
        let scalar = SlotObservations {
            slot: "s".into(),
            values: SlotValues::Scalar((0..100).map(|i| i as f64).collect()),
        };
        let col = discretize_slot(&scalar, 1, &cfg).unwrap();
        assert_eq!(col.len(), 100);
        assert!(col.iter().all(|&b| (0..4).contains(&b)));

        let label = SlotObservations {
            slot: "l".into(),
            values: SlotValues::Label(vec![10, 10, 20, 30, 20]),
        };
        let col = discretize_slot(&label, 1, &cfg).unwrap();
        assert_eq!(col, vec![0, 0, 1, 2, 1]);
    }

    #[test]
    fn empty_slot_is_rejected() {
        let cfg = DiffConfig::from_defaults().unwrap();
        let empty = SlotObservations {
            slot: "e".into(),
            values: SlotValues::Scalar(vec![]),
        };
        let err = discretize_slot(&empty, 1, &cfg).unwrap_err();
