...
mod tests {
    ...
    fn subj(byte: u8, stratum: &str) -> AssaySubject {
        AssaySubject::new(SeriesId::from_bytes([byte; 16]), stratum, [byte; 32]).unwrap()
    }

    #[test]
    fn apportionment_sums_to_target_and_respects_sizes() {
        // Strata of 50, 30, 20; sample 10 -> exact 5,3,2 with no remainder.
        let seats = largest_remainder_apportionment(&[50, 30, 20], 10);
        assert_eq!(seats, vec![5, 3, 2]);
        assert_eq!(seats.iter().sum::<usize>(), 10);
    }

    #[test]
    fn apportionment_uses_largest_remainder_for_leftover_seats() {
        // Strata 6,6,1 (N=13), sample 7. quotas = 42/13=3.23, 3.23, 7/13=0.538.
        // base = 3,3,0 (sum 6), leftover 1 -> largest remainder is stratum 2
        // (0.538) so it gets the seat: 3,3,1.
        let seats = largest_remainder_apportionment(&[6, 6, 1], 7);
        assert_eq!(seats, vec![3, 3, 1]);
    }

    #[test]
    fn apportionment_clamps_to_population_when_sample_exceeds_it() {
        let seats = largest_remainder_apportionment(&[3, 2], 100);
        assert_eq!(seats, vec![3, 2]);
    }

    #[test]
    fn sampling_is_order_independent_and_seed_stable() {
        let forward = Population::new(
            (0..10)
                .map(|i| subj(i, "a"))
                .chain((10..14).map(|i| subj(i, "b"))),
        )
        .unwrap();
        let reverse = Population::new(
            (0..14)
                .rev()
                .map(|i| subj(i, if i < 10 { "a" } else { "b" })),
        )
        .unwrap();
        let ra = stratified_sample(&forward, 42, 7, 1, &[]);
        let rb = stratified_sample(&reverse, 42, 7, 1, &[]);
        assert_eq!(ra, rb);
        // 10:4 population, sample 7 -> quotas 5.0, 2.0 -> exactly 5,2.
        assert_eq!(
            ra.allocations,
            vec![
                StratumAllocation {
        ...
        assert_eq!(ra.effective_sample_size, 7);
        assert_eq!(ra.rows.len(), 7);
    }

    #[test]
    fn different_seed_can_select_different_subjects() {
        let pop = Population::new((0..20).map(|i| subj(i, "a"))).unwrap();
        let a = stratified_sample(&pop, 1, 5, 1, &[]);
        let b = stratified_sample(&pop, 2, 5, 1, &[]);
        let sel_a: Vec<_> = a.rows.iter().map(|r| r.series_id).collect();
        let sel_b: Vec<_> = b.rows.iter().map(|r| r.series_id).collect();
        // Overwhelmingly likely to differ; assert the seed actually influences selection.
