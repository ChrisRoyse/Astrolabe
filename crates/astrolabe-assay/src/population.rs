...
mod tests {
    ...
    fn series(byte: u8) -> SeriesId {
        SeriesId::from_bytes([byte; 16])
    }

    #[test]
    fn empty_stratum_is_rejected() {
        let err = AssaySubject::new(series(1), "", [0; 32]).unwrap_err();
        assert_eq!(err.code(), ASTRO_ASSAY_EMPTY_STRATUM);
    }

    #[test]
    fn population_is_order_independent() {
        let a = AssaySubject::new(series(1), "function", [1; 32]).unwrap();
        let b = AssaySubject::new(series(2), "class", [2; 32]).unwrap();
        let c = AssaySubject::new(series(3), "function", [3; 32]).unwrap();
        let forward = Population::new([a.clone(), b.clone(), c.clone()]).unwrap();
        let reverse = Population::new([c, b, a]).unwrap();
        assert_eq!(forward, reverse);
        assert_eq!(
            forward.strata_sizes(),
            vec![("class".to_string(), 1), ("function".to_string(), 2)]
        );
    }
    ...
    fn duplicate_identity_is_rejected() {
        let a = AssaySubject::new(series(1), "function", [1; 32]).unwrap();
        let dup = AssaySubject::new(series(1), "function", [9; 32]).unwrap();
        let err = Population::new([a, dup]).unwrap_err();
        assert_eq!(err.code(), ASTRO_ASSAY_DUPLICATE_SUBJECT);
    }
}
