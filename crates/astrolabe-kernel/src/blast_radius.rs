...
mod tests {
    use super::*;
    use crate::kernel_graph::{KernelGraphEdge, KernelGraphNode};

    fn cx(n: u8) -> CxId {
        CxId::from_bytes([n; 16])
    }

    /// A -> B (w=1.0), B -> C (w=1.0), A -> D (w=0.5). Node D is external to the
    /// kernel; A and B are grounded members; C is an ungrounded member (a gap).
    fn fixture_graph() -> KernelGraph {
        let nodes = vec![
            KernelGraphNode::new(cx(1), 1, None), // A
            KernelGraphNode::new(cx(2), 1, None), // B
            KernelGraphNode::new(cx(3), 1, None), // C
            KernelGraphNode::new(cx(4), 1, None), // D
        ];
        let edges = vec![
            KernelGraphEdge::new(cx(1), cx(2), 1.0),
            KernelGraphEdge::new(cx(2), cx(3), 1.0),
            KernelGraphEdge::new(cx(1), cx(4), 0.5),
        ];
        KernelGraph::new(nodes, edges).expect("fixture graph")
    }

    #[test]
    fn reach_golden_is_hand_computable() {
        let graph = fixture_graph();
        let members: BTreeSet<CxId> = [cx(1), cx(2), cx(3)].into_iter().collect();
        let gaps: BTreeSet<CxId> = [cx(3)].into_iter().collect();
        let config = ReachConfig::with_registry_defaults();

        let reach = change_reach(&graph, cx(1), &members, &gaps, &config).expect("reach");

        // Hop 1: B = floor(1000 · 1000/1000 · 900/1000) = 900
        //        D = floor(1000 ·  500/1000 · 900/1000) = 450
        // Hop 2: C = floor( 900 · 1000/1000 · 900/1000) = 810
        let by_id: BTreeMap<CxId, ReachedNode> = reach.reached.iter().map(|n| (n.id, *n)).collect();
        assert_eq!(by_id[&cx(2)].reach_permille, 900);
        assert_eq!(by_id[&cx(2)].hop, 1);
        assert_eq!(by_id[&cx(4)].reach_permille, 450);
        assert_eq!(by_id[&cx(4)].hop, 1);
        assert_eq!(by_id[&cx(3)].reach_permille, 810);
        assert_eq!(by_id[&cx(3)].hop, 2);

        assert_eq!(reach.reached_count, 3);
        assert_eq!(reach.reach_mass_permille, 900 + 450 + 810);
        assert_eq!(reach.kernel_member_reach_permille, 900 + 810); // B + C
        assert_eq!(reach.gap_reach_permille, 810); // only C is a gap
        assert!(reach.from_is_kernel_member);
        assert!(!reach.from_is_gap);
        assert!(by_id[&cx(2)].kernel_member && !by_id[&cx(2)].gap);
        assert!(by_id[&cx(3)].kernel_member && by_id[&cx(3)].gap);
        assert!(!by_id[&cx(4)].kernel_member);
    }

    #[test]
    fn reach_risk_elevation_golden() {
        let graph = fixture_graph();
        let members: BTreeSet<CxId> = [cx(1), cx(2), cx(3)].into_iter().collect();
        let gaps: BTreeSet<CxId> = [cx(3)].into_iter().collect();
        let config = ReachConfig::with_registry_defaults();
        ...
        // base=200, gap_exposure=min(1000,810)=810, gap_term=floor(810·700/1000)=567,
        // membership_bonus=300 (from is a member) => blend=min(1000,867)=867.
        // risk = 200 + floor((1000-200)·867/1000) = 200 + floor(693.6) = 893.
        let risk = reach_risk_permille(200, &reach, &config).expect("risk");
        assert_eq!(risk.gap_exposure_permille, 810);
        assert_eq!(risk.elevation_permille, 867);
        assert!(risk.kernel_membership_applied);
        assert_eq!(risk.risk_permille, 893);
        // Monotone: risk never drops below the oracle base and never exceeds 1000.
        assert!(risk.risk_permille >= 200 && risk.risk_permille <= 1000);
    }

    #[test]
    fn reach_is_deterministic_byte_identical() {
        let graph = fixture_graph();
        let members: BTreeSet<CxId> = [cx(1), cx(2), cx(3)].into_iter().collect();
        let gaps: BTreeSet<CxId> = [cx(3)].into_iter().collect();
        let config = ReachConfig::with_registry_defaults();
        let a = change_reach(&graph, cx(1), &members, &gaps, &config).expect("a");
        let b = change_reach(&graph, cx(1), &members, &gaps, &config).expect("b");
        assert_eq!(
            change_reach_artifact_bytes(&a),
            change_reach_artifact_bytes(&b)
        );
    }

    #[test]
    fn leaf_change_has_no_reach_and_no_elevation_without_membership() {
        // A change on external leaf D reaches nothing; if D is not a kernel member
        // the risk equals the oracle base exactly (byte-compatible with today).
        let graph = fixture_graph();
        let members: BTreeSet<CxId> = [cx(1), cx(2), cx(3)].into_iter().collect();
        let gaps: BTreeSet<CxId> = [cx(3)].into_iter().collect();
        let config = ReachConfig::with_registry_defaults();
        let reach = change_reach(&graph, cx(4), &members, &gaps, &config).expect("reach");
        assert_eq!(reach.reached_count, 0);
        assert_eq!(reach.gap_reach_permille, 0);
        assert!(!reach.from_is_kernel_member);
        let risk = reach_risk_permille(350, &reach, &config).expect("risk");
        assert_eq!(risk.elevation_permille, 0);
        assert_eq!(risk.risk_permille, 350, "no reach => oracle base unchanged");
    }

    #[test]
    fn unknown_symbol_refuses_fail_closed() {
        let graph = fixture_graph();
        let config = ReachConfig::with_registry_defaults();
        let error = change_reach(&graph, cx(200), &BTreeSet::new(), &BTreeSet::new(), &config)
            .expect_err("unknown symbol must refuse");
        assert_eq!(error.code(), ASTRO_CHANGE_REACH_UNKNOWN_SYMBOL);
        assert!(!error.remediation().is_empty());
    }

    #[test]
    fn knob_out_of_bounds_refuses() {
        let mut config = ReachConfig::with_registry_defaults();
        config.max_hops = 99;
        let graph = fixture_graph();
        let error = change_reach(&graph, cx(1), &BTreeSet::new(), &BTreeSet::new(), &config)
            .expect_err("out-of-bounds hop budget must refuse");
        assert_eq!(error.code(), ASTRO_CHANGE_REACH_KNOB_RANGE);
    }
}
