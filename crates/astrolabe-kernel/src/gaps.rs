...
mod tests {
    use super::*;
    use crate::kernel_build::{KernelBuildConfig, KernelMember, RecallMeasurement};
    ...
    fn member(
        ...
        KernelMember {
            ...
            score_permille,
            degree: 0,
            betweenness_permille: 0,
            groundedness_permille,
            frequency,
            grounded,
            in_fvs: true,
            support_added: false,
    ...
    /// Assembles a kernel artifact around a hand-chosen member set for testing the
    /// pure report/quadrant functions. `anchor_grounded` mirrors whether any
    /// member is grounded.
    fn artifact(members: Vec<KernelMember>) -> KernelArtifact {
        ...
        KernelArtifact {
            ...
            recall: RecallMeasurement {
                recalled: members.len() as u64,
                total: members.len() as u64,
                permille: 1000,
                gated: true,
            },
            members,
            members_hash: "0".to_string(),
    ...
    #[test]
    fn gap_report_selects_only_ungrounded_members() {
        // Two grounded, two ungrounded members: only the ungrounded ones are gaps.
        let art = artifact(vec![
            member(1, 800, 3, 900, true),
            member(2, 700, 5, 100, false),
            member(3, 600, 2, 950, true),
            member(4, 400, 4, 50, false),
        ]);
        let report = grounding_gap_report(&art);
        assert_eq!(report.member_count, 4);
        assert_eq!(report.gap_count, 2);
        let ids: Vec<CxId> = report.gaps.iter().map(|gap| gap.id).collect();
        assert_eq!(ids, vec![cx(2), cx(4)]);
    ...
    #[test]
    fn gap_ranking_golden_is_score_times_churn() {
        // Hand-computed rank = score × churn:
        //   A id=1: 500×2 = 1000
        //   B id=2: 300×4 = 1200
        //   C id=3: 600×2 = 1200   (ties B; id 3 > id 2 => B first)
        //   D id=4: 900×1 =  900
        // Expected order: B(1200), C(1200), A(1000), D(900).
        let art = artifact(vec![
            member(1, 500, 2, 0, false),
            member(2, 300, 4, 0, false),
            member(3, 600, 2, 0, false),
            member(4, 900, 1, 0, false),
        ]);
        let report = grounding_gap_report(&art);
        let ordered: Vec<(CxId, u128)> = report
            .gaps
            .iter()
            .map(|gap| (gap.id, gap.rank_score))
            .collect();
        assert_eq!(
            ordered,
            vec![(cx(2), 1200), (cx(3), 1200), (cx(1), 1000), (cx(4), 900),]
        );
    }

    #[test]
    fn gap_report_all_ungrounded_when_no_anchor() {
        let art = artifact(vec![
            member(1, 500, 1, 0, false),
            member(2, 600, 1, 0, false),
        ]);
        let report = grounding_gap_report(&art);
        assert_eq!(report.gap_count, 2);
        assert!(!report.has_trusted_anchor);
    ...
    #[test]
    fn quadrant_classification_is_hand_computable() {
        let config = QuadrantConfig {
            importance_threshold_permille: 500,
            anchor_density_threshold_permille: 500,
        };
        // id=1 score 800 density 100 => critical & unverified (dragons)
        // id=2 score 900 density 700 => critical & covered
        // id=3 score 200 density 100 => peripheral & unverified
        // id=4 score 100 density 800 => peripheral & covered
        // id=5 score 500 density 500 => boundary (>= threshold both) => critical & covered
        let art = artifact(vec![
            member(1, 800, 3, 100, false),
            member(2, 900, 2, 700, true),
            member(3, 200, 1, 100, false),
            member(4, 100, 1, 800, true),
            member(5, 500, 1, 500, true),
        ]);
        let quadrant = coverage_importance_quadrant(&art, &config).unwrap();
        assert_eq!(quadrant.critical_unverified_count, 1);
        assert_eq!(quadrant.critical_covered_count, 2);
        assert_eq!(quadrant.peripheral_unverified_count, 1);
        assert_eq!(quadrant.peripheral_covered_count, 1);

        let dragons: Vec<CxId> = quadrant.critical_unverified().map(|p| p.id).collect();
        assert_eq!(dragons, vec![cx(1)]);
        assert_eq!(quadrant.trust, "verified");

        // The dragons quadrant leads the scatter.
        assert_eq!(quadrant.points[0].id, cx(1));
        assert_eq!(quadrant.points[0].quadrant, GapQuadrant::CriticalUnverified);
    }

    #[test]
    fn quadrant_recompute_is_byte_identical() {
        let config = QuadrantConfig::with_registry_defaults();
        let art = artifact(vec![
            member(7, 800, 3, 100, false),
            member(2, 900, 2, 700, true),
            member(9, 200, 1, 100, false),
        ]);
        let first = coverage_importance_quadrant(&art, &config).unwrap();
        let second = coverage_importance_quadrant(&art, &config).unwrap();
        assert_eq!(
            coverage_importance_quadrant_artifact_bytes(&first),
            coverage_importance_quadrant_artifact_bytes(&second)
        );
        // And the report likewise.
        let report = grounding_gap_report(&art);
        let report2 = grounding_gap_report(&art);
        assert_eq!(
            grounding_gap_report_artifact_bytes(&report),
            grounding_gap_report_artifact_bytes(&report2)
        );
    }

    #[test]
    fn quadrant_config_validates_bounds() {
        let bad = QuadrantConfig {
            importance_threshold_permille: 1001,
            anchor_density_threshold_permille: 500,
        };
        let err = bad.validate().unwrap_err();
        assert_eq!(err.code(), ASTRO_GAP_QUADRANT_KNOB_RANGE);
    }
}
