...
fn rows_from_export(export: &TeamVaultExport) -> LowerResult<Vec<LedgerRow>> {
    ...
    for row in &export.ledger_rows {
        ...
        let bytes = decode_hex(&row.bytes_hex, "ledger row bytes")?;
...
fn ensure_intact_chain(store: &MemoryLedgerStore, range_end: u64) -> LowerResult<()> {
    match calyx_verify_chain(store, 0..range_end)? {
        ...
        VerifyResult::Broken { at_seq, .. } => Err(invalid(
            ASTRO_TEAM_ARTIFACT_LEDGER_TAIL,
            format!("ledger chain broken at seq {at_seq}"),
        )),
        VerifyResult::Corrupt { at_seq, reason } => Err(invalid(
            ASTRO_TEAM_ARTIFACT_LEDGER_TAIL,
...
fn decode_hex_32(input: &str, label: &str) -> LowerResult<[u8; 32]> {
    let bytes = decode_hex(input, label)?;
    let len = bytes.len();
...
fn decode_hex(input: &str, label: &str) -> LowerResult<Vec<u8>> {
...
fn hex_nibble(byte: u8, label: &str) -> LowerResult<u8> {
    match byte {
        b'0'..=b'9' => Ok(byte - b'0'),
        ...
        _ => Err(invalid(
            ASTRO_TEAM_ARTIFACT_VAULT_BYTES,
            format!("{label} contains non-hex bytes"),
        )),
    }
}

fn invalid(code: &'static str, message: impl Into<String>) -> LowerError {
    LowerError::InvalidInput(format!("{code}: {}", message.into()))
}
