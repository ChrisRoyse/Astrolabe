...
> Standing in this repository: this is the binding upstream doctrine that
> `docs/astrolabe-blueprint.md` §01.2 cites as "the Calyx handbook." The blueprint applies this
> handbook to the code domain; where the two disagree, the blueprint's code-domain decisions win,
> but every deviation must be recorded there as a design correction, never silently. Constants
> below are Calyx defaults; ASTROLABE treats them as registry-declared knobs or measured values
> per standing invariant 4.
...
>
> **Where meaning comes from.** Meaning is *derived from associations* — how bits associate with vectors, how vectors associate with the underlying measurements, how records associate with outcomes, and every other measurable relation in the web. An atom in isolation has no meaning; **the meaning of anything is the culmination of all the grounded intelligence signals about it.** Ungrounded association structure is geometry — real, computable, but not yet meaning; an association becomes *meaning* only when it is grounded, i.e., carries measured bits about an anchored real outcome. This makes meaning **quantitative and situational**: "this works," "this matters," "this is X" is established by the presence of some number of independent grounded signals, and that number differs for every situation — a low-stakes ranking nudge may need one weak signal; an identity decision may need many strong ones (the sufficiency test `I(panel; anchor) ≥ H(anchor)` is exactly the formal version of "enough grounded signal has accumulated for this outcome"). The corollary is the maximization rule: **to derive the most meaning from anything, accumulate the most — and most diverse — grounded intelligence you can about it.** More anchors per record, more anchor kinds per experiment, more independent lenses measured against the same outcomes ⇒ more bits ⇒ more derivable meaning. Grounded intelligence is not a validation step bolted onto the pipeline; it is the substance the pipeline exists to accumulate.
>
> **Therefore, never shortcut it.** If you skip the decomposition to atoms, or leave base associations uncomputed, the intelligence you extract is partial, biased, and ungrounded — you will have measured a shadow of the system, not the system. The discipline is always the same: **atoms → all base associations → differentiate → kernel → compose.** Decompose everything; compute every association from the ground up; only then does Calyx have the full information it needs to derive maximum grounded intelligence — and only what is grounded in that structure counts as meaning.
...
3. **Meaning is accumulated, not assigned — so maximize grounded signal density.** A record's meaning is the culmination of the grounded signals about it (★ above). Practically: every experiment, ablation, eval, or observation you run should harvest **every** grounded signal it already pays for — one intervention typically yields several independent outcome measurements (a delta on each metric, per stratum, per axis), and each is a separate anchor at near‑zero marginal cost. Anchor floors ("every record has ≥ 1 anchor") are floors, not targets; the target is the richest grounded panel the budget allows, because bits about outcomes — not vectors — are what the kernel, the guard, the oracle, and every answer are ultimately made of. "How many grounded signals are enough" (the X) is situational — let the sufficiency test and the honesty gate, not habit, decide it per outcome.
...
- **Anchor density is the cheapest lever.** One intervention or observation usually yields several independent grounded measurements (per metric, per stratum, per axis) — record them ALL as separate anchors; the marginal cost is near zero and every added grounded signal raises the meaning derivable from the same corpus (★). Audit periodically: records with rich vectors but thin anchors are geometry waiting to become meaning.
...
- **Grounding mandatory — and meaning IS accumulated grounding.** Measure against anchored outcomes; ungrounded ⇒ *provisional*. The meaning of any record/claim/capability is the culmination of the grounded signals about it; the number needed (X) is situational and adjudicated by sufficiency + the honesty gate, never by habit. Maximize grounded signal density: harvest every outcome measurement an experiment already pays for; treat anchor minimums as floors, not targets. Gates use a real **diverse** panel; a single embedder or synthetic check is diagnostic only.
