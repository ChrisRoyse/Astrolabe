...
- Multi-agent/process behavior: all processes may serve legacy SQLite reads; shadow imports share one Aster vault and serialize durable commits through `locks/durable.commit.lock`; lowered SQLite sidecar regeneration serializes through a per-project `.astrolabe-lowered.lock`. No watcher/anneal/background lane is active in followers until single-owner election lands.
