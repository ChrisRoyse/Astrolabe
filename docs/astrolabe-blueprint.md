...
- Ledger payloads are content-neutral bounded domain data: writers own structural encoding, while the ledger owns deterministic hashing, append-only pairing, and chain verification. Explicit raw-input retention and actor-name redaction remain available where a product writer chooses them.
...
Redaction audit status: `ci/redaction-writers.json` plus `scripts/check-redaction-writers.py` enumerate all Astrolabe production ledger writer call sites and require a payload contract plus coverage note for each new writer. `astrolabe-ingest` now proves both direct append and batch-with-ledger writes reject secret-shaped payloads before ledger/data rows land. Shell-arg audit status: `ci/shell-arg-audit.json` plus `scripts/check-shell-arg-audit.py` enumerate shell-sensitive production bridge calls and require the inherited CBM-style validator before the call; `astrolabe-bridge` exercises injection-shaped root paths against the actual watcher boundary. UI loopback status: `scripts/check-lowered-parity.py --ui-smoke` reads the live listener table for the spawned graph UI port and fails on any non-loopback bind. Egress audit status: `scripts/check-egress-deny.py` runs shadow index plus stdio serving under `strace` `connect(2)` denial and fails on any blocked outbound connect or non-loopback bind. Append-only erasure proof status: `astrolabe-ingest` erases an imported Cx in a durable Aster vault, byte-compares pre-existing ledger rows, decodes the appended erasure tombstone, and verifies the chain remains intact. Lowered-artifact erasure proof status: `astrolabe-lower` imports a sentinel-bearing CBM fixture, proves the pre-erasure lowered SQLite and FTS contain the sentinel, erases the imported Cx through Calyx, tombstones Astrolabe-owned Graph node/edge rows with `erase_imported_cx_graph_rows`, regenerates lowered SQLite, and verifies zero table/FTS hits, no sentinel bytes in the regenerated artifact, intact chain state, and no non-WAL durable vault file retaining the sentinel. Active WAL bytes remain labeled retained history under Calyx's WAL contract. Remaining #61 work is WAL-recycler/full-physical-history erasure policy plus cached packs once those packs are instantiated.
...
Implementation status: `optimizer_status` runs a cooperative janitor tick over the Astrolabe-owned `<project>.astrolabe-optimizer-artifacts` cache root, reports the 100 MiB/tick policy as `max_bytes_per_tick`, deletes only regular files without following symlinks, and reports before/after bytes, skip counts, `freshness`, `trust`, and remediation. `mode="ack_triggers"` appends an Astrolabe-owned ack ledger entry naming exact fired events for one subscription, then reopens trigger readback to prove pending count changes without deleting Reactive CF fired rows. `mode="propose"` reads measured `astrolabe.optimizer_deficits.v1` rows, refuses under optimizer freezes, generates pending proposal candidates, writes `astrolabe.optimizer_proposals.v1`, and rereads the config row before returning success. Persisted `astrolabe.optimizer_guard_health.v1`, `astrolabe.optimizer_tripwires.v1`, and `astrolabe.optimizer_proposals.v1` profiles are schema-validated and surfaced only when each row carries measured values, `freshness`, `trust`, and provenance. Proposal mode still stops at a pending differentiation gate; P8.3 shadow/admission/hot-add remains separate. Coverage reads remaining janitor file bytes, durable ack ledger payloads after reopen, raw guard-health `_config.db` bytes, raw tripwire `_config.db` bytes, raw proposal queue `_config.db` bytes, and freeze-refusal state. Remaining #58 work is guard_calibrate/tripwire profile production.
