...
| **`get_architecture`** | High-level map: packages, services, dependencies, routes, layers, boundaries, entry points, hotspots, and **Leiden clusters** (the de-facto modules, which often cut across folders) | `project`, `path`, `aspects` |
...
| **`detect_changes`** ✨ | What changed vs a base ref, and what it impacts | `project`, `scope`, `depth` (2), `base_branch`, `since` |
...
Omitting an extension keeps byte-identical legacy behavior — that parity is deliberate, so you can always fall back.
...
| Tool | Use it to |
|---|---|
| **`get_kernel`** | Serve the distilled kernel — the feedback-vertex-set of symbols that dominate the codebase's association graph. Four modes: `read` (members + recall per scope), `gaps` ("here be dragons": important-but-ungrounded members ranked by weight), `quadrant` (coverage × importance scatter; **critical-and-unverified is your QA target list**), `build` (recompute and persist on demand). Optional `scope`; a `fleet:*` scope serves a cross-repo composed kernel |
...
| Tool | Use it to |
|---|---|
| **`predict_impact`** | *"If I change X, what breaks?"* Composite consequence graph from real edges, grounded in the vault's occurrence rows, cycle-guarded butterfly walk (×0.7/hop, prune <0.05, depth ≤4), three independent ceilings keeping probability strictly < 1.0. Consequences intersecting `TESTS` edges become a **ranked test-selection set**. `mode:"backtest"` runs and persists this repo's grounded-vs-topology gate |
...
**Goal:** go from zero knowledge to a correct mental model in ~4 calls instead of 40 file reads.

```
1. index_repository { repo_path, mode: "full", calyx: "shadow" }
2. get_architecture  { project, aspects: ["overview", "clusters", "entry_points", "boundaries"] }
3. get_kernel        { project, mode: "read" }
4. get_kernel        { project, mode: "quadrant" }
```

**How to read the results:**

- `get_architecture.clusters` is the highest-value block most agents skip. Leiden community detection over the call/import graph surfaces the **de-facto modules** — which routinely disagree with the folder layout. When the clusters and the directory tree disagree, the clusters are the truth about coupling, and that disagreement is itself a finding worth reporting.
...
```
5. kernel_answer { project, query: "how does request authentication work?" }
```

If it refuses with a deficit, that is informative: the codebase has no grounded path for that question yet. Fall back to `search_graph` + `trace_path`, and consider `get_kernel mode:"build"` if the refusal names a missing kernel.
...
- Grounded confidence is advertised **only** when the repo's persisted backtest gate passed. Otherwise every consequence is labeled `provisional` — never a silent grounded default. Run `predict_impact mode:"backtest"` once per repo to establish that gate.
- If `predict_impact` refuses with a deficit card, the seeds have no grounded history. Do not read that as "safe to change" — read it as "no evidence either way," and fall back to `trace_path` topology plus caution.
...
```
1. detect_changes    { project, since: "<base>", depth: 2 }   → grounded_risk per impacted symbol
...
```
predict_impact { project, mode: "backtest" }   → persists the grounded-vs-topology gate
...
Without the backtest gate, `predict_impact` labels everything provisional. Without the calibrated profile, every guard tool refuses. Without a kernel, `kernel_answer` refuses and tells you to build one.
...
- **macOS / Apple Silicon is the primary, evidence-bearing host** (owner directive, 2026-08-01, superseding the retired Windows-only scope). An issue closes on native macOS FSV. Windows and Linux remain supported targets and their platform branches stay in the tree, but they are no longer the evidence authority and no longer gate any DoD. Portability work is ordinary in-scope work, not a deferred final phase.
...
└── isError: true
    ...
    ├── code names a missing gate    → run the gate (backtest / calibrate / kernel build), retry once
    ├── code = *DEFICIT / Insufficient → do NOT retry; report the deficit and fall back to
    │                                     topology-only reasoning, saying so explicitly
...
```
index_status → get_kernel mode:"read" → get_architecture aspects:["overview","clusters"]
```
...
| Document | Role |
|---|---|
| [`README.md`](README.md) | What ASTROLABE is; native macOS / Apple Silicon build instructions |
| [`CLAUDE.md`](CLAUDE.md) | Agent operating manual for *developing* ASTROLABE (workflow, hygiene) |
| [`docs/astrolabe-blueprint.md`](docs/astrolabe-blueprint.md) | Plan of record — the full 23-part design and capability catalog |
| [`docs/BUILDING_ON_CALYX.md`](docs/BUILDING_ON_CALYX.md) | Binding upstream doctrine (embed-vs-encode, the anti-pattern list) |
| [`docs/status/`](docs/status/) | Snapshot of delivered vs remaining scope |
