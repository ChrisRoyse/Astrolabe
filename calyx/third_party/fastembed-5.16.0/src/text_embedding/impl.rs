...
impl TextEmbedding {
    ...
    pub fn try_new(options: TextInitOptions) -> Result<Self> {
        ...
        let mut builder = Session::builder()?
            ...
            .map_err(Self::builder_error)?;
        builder = session_policy.apply_to(builder)?;
    ...
    pub fn try_new_from_user_defined(
        ...
        let session = {
            let mut session_builder = Session::builder()?
                ...
                .with_intra_threads(threads)
                .map_err(Self::builder_error)?;
            session_builder = session_policy.apply_to(session_builder)?;
    ...
    /// Read-only access to the committed session for provider-placement attestation.
    pub fn session(&self) -> &ort::session::Session {
        &self.session
    }

    /// Finalize and return the graph-assignment profiling trace path.
