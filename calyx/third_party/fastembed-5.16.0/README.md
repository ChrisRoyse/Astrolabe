...
Every ONNX-backed constructor in Astrolabe's owned FastEmbed build requires an explicit
`SessionPolicy` and exactly one matching execution provider. Default initialization options
...
let image_embeddings = model.embed_images(&["assets/image_0.png", "assets/image_1.png"])?;
...
```rust
use fastembed::{SessionPolicy, SparseEmbedding, SparseInitOptions, SparseModel, SparseTextEmbedding};
use ort::ep::CPU;
...
```rust
use fastembed::{Bgem3Embedding, Bgem3InitOptions, Bgem3Model, SessionPolicy};
use ort::ep::CPU;
...
```rust
use fastembed::{
    Bgem3Embedding, InitOptionsUserDefined, SessionPolicy, TokenizerFiles,
...
```rust
use fastembed::{ImageEmbedding, ImageEmbeddingModel, ImageInitOptions, SessionPolicy};
use ort::ep::CPU;
...
```rust
use fastembed::{RerankInitOptions, RerankerModel, SessionPolicy, TextRerank};
use ort::ep::CPU;
...
Astrolabe's owned session contract accepts only an explicitly paired CPU or fail-closed CUDA
provider. Other ONNX Runtime providers, including DirectML, are rejected with a structured
`FASTEMBED_SESSION_POLICY[PROVIDER_TYPE_MISMATCH]` error because this build does not attest their
graph placement.
