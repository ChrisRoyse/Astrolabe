...
> **ARCHIVED SOURCE SNAPSHOT, NOT CURRENT ASTROLABE DOCUMENTATION.** The combined
> series was generated from the former `Calyx-Dev` tree at commit `2e7b9a2` and
> intentionally preserves historical behavior, including removed Candle
> precision replay. Use root `AGENTS.md`/`CLAUDE.md` for operations and
> `calyx/docs/systemspecs/` for current Calyx behavior. Current Candle/Qwen
> runtimes require explicit dtype, placement-only auto selection, verified
> safetensors source profiles, real full-forward activation/device attestation,
> and no dtype or CPU retry.
...
| CLI | `CALYX_CLI_USAGE_ERROR`, `CALYX_CLI_IO_ERROR`, `CALYX_CLI_RUNTIME_ERROR` |
...
`OnnxLens` (fastembed/ort), `StaticLookupLens` (authenticated mmap model2vec, `CXLKUP2`),
...
`modality`, `dim`, `timeout` (default 30s), `max_batch` (default 64, min 1).
...
`DEFAULT_TEI_ENDPOINT = "http://127.0.0.1:18190/embed"` for the Calyx-owned resident FP16 E5 lane; `:8088` is legacy/shared compatibility only.
...
TEI default endpoint for `text-default` slots: `http://127.0.0.1:18190`.
...
| ID | Name | Fault injected | Key invariants |
|---|---|---|---|
| 22 | request-text non-persistence | inject a marker into rerank, embed, and search request text; append hash-only ledger entries | marker never on disk (scan before/after empty); ledger payloads hash-only; `RerankRequest` Debug omits request text; rerank score 0.42 |
...
- `lens add --manifest <m.json> [--home <dir>]` — imports a manifest into the Calyx/Aster lens catalog DB (dedup by `lens_id`); legacy `{home}/lenses/registry.json` is migration input only.
...
| `tei_endpoints` | Vec<String> | no | `[]` | Documents `:18190/:18188/:8088/:8089/:8090`. |
