...
The `calyx-registry` crate is the lens/embedder registry: it defines the `Lens` measurement trait, algorithmic, TEI HTTP, Candle, Qwen3, ONNX/FastEmbed, static-lookup, external-command, multimodal-adapter, and temporal runtime families, the frozen content-addressed lens contract and `LensId`, hot-swap + backfill machinery, capability cards + gating, default domain panels, vault persistence, and vector compression. All claims below are traced to source.
...
src/runtime/common.rs            shared helpers and local execution attestation
...
src/runtime/qwen3/*              FastembedQwen3Lens (local Qwen3 embedding)
...
src/commission.rs (+manifest/source_tensor_profile) LensForge manifest and safetensors profile
...
| `CandleLens` | `runtime/candle.rs` | `Dense(dim)` | `Text` | tokenizes, runs one mutex-guarded BERT forward, checks frozen dtype/device, converts output to F32, pools, normalizes | construction first performs a real full-forward hidden-tensor dtype/device/finite attestation; no precision replay or CPU retry |
| `FastembedQwen3Lens` | `runtime/qwen3/*` | `Dense(dim)` | `Text` | tokenizes, runs Qwen3 embedding inference, last-token pools, normalizes | all-shard profile plus real full-forward hidden-tensor dtype/device/finite attestation; no precision replay or CPU retry |
...
**CandleLens** — `DEFAULT_CANDLE_MODEL = "sentence-transformers/all-MiniLM-L6-v2"` (`candle.rs`). Enums in `candle/options.rs`: `CandleDeviceMode { Auto, Cuda, Cpu }`; `CandleDevicePolicy { CpuExplicit, CpuNoCudaFeature, CpuNoCudaDevice, CudaFailLoud { ordinal } }`; `CandlePrecision { F32, F16, BF16 }`; `CandlePoolingPolicy { Mean, Cls }`. Every public constructor requires an explicit precision. Auto selects placement only: it chooses CPU only when CUDA was not compiled or the CUDA driver reports no device, never changes the explicit dtype, and rejects half precision if it resolves CPU. Any CUDA initialization, ordinal, allocation, model-load, attestation, or inference failure after CUDA selection is fatal; there is no second-precision replay or CPU retry. Persisted specs freeze `cpu` or `cuda:<ordinal>` and CPU contracts require F32. Model config bytes are used exactly, with no layer-norm epsilon rewrite. After artifact hashes are verified, a canonical safetensors source-tensor dtype profile is recomputed from the physical weight set and its fingerprint enters lens identity. `VarBuilder` receives the declared dtype as its loader target; model construction then runs a real full forward pass and requires the primary hidden tensor to have that dtype, reside on the frozen device, and contain only finite values. Successful telemetry reports `evidence_kind=full_forward_hidden_tensor`. All three Candle reduced-precision GEMM switches are centrally held false and verified before/after load and inference, so F16/BF16/F32 model execution uses F32 accumulation and emits F32 slot vectors. Pooling/normalization live in `candle/pooling.rs`. The v6 contract hashes model id, max tokens, device, model precision, pooling, norm, source-profile fingerprint, and the exact-execution/full-forward-attestation policy.

**FastembedQwen3Lens** — `runtime/qwen3/*` owns the local Qwen3 embedding path. It uses the same explicit `CandlePrecision` and placement-only `CandleDevicePolicy` contract as Candle, requires F32 for CPU, preserves Qwen's fixed left-padding/last-token/L2 policy, and rejects mixed or incomplete safetensors shard sets. The canonical source-tensor profile covers every shard and rejects duplicate tensor names. Model construction uses the declared dtype as the loader target, executes one real Qwen3 full forward pass, and requires the primary hidden activation's dtype/device/finite readback to match before exposing the lens. GEMM accumulation and stored slot output remain F32. The `fastembed-qwen3-text-v4` corpus contract includes model id, frozen device, explicit precision, max tokens, source-profile fingerprint, exact config, and full-forward attestation; it never retries another dtype or CPU.
...
**StaticLookupLens** — memory-maps an authenticated `CXLKUP2` vocabulary matrix. Its 64-byte header binds non-zero `rows`, `dim`, explicit dtype (`I8=1`, `F16=2`, `F32=3`), `vocab_size == rows`, checked body length, and a BLAKE3 body digest. INT8 stores one finite positive symmetric scale per row followed by canonical `[-127,127]` codes; F16/F32 store finite little-endian values. `StaticLookupWriter` derives row count from the real tokenizer, accepts one source row at a time, seals and production-reader-verifies a staged artifact, and publishes it immutably without whole-matrix materialization. Lookup accumulates directly from the mmap through an attested AVX2 backend (F16 additionally requires F16C), or an explicitly reported scalar backend on other CPUs. Empty/all-unknown inputs fail with `CALYX_LENS_EMPTY_PROJECTION`; out-of-range token ids, checksum/shape/vocabulary drift, noncanonical INT8, and legacy `CXLKUP1` all fail closed. `DEFAULT_MAX_TOKENS = 512`.
...
Default TEI endpoint used by templates: `http://127.0.0.1:18190`.
...
`LensForgeManifest` describes a forged embedder: `name, modality, runtime (string), dim (>0), shape?, dtype, execution_device?, weights_sha256, artifact_set_sha256?, files: Vec<LensForgeFile>, pooling, norm, source_hf_id, license?, non_commercial, quant_default, truncate_dim?, recall_delta, max_batch?, batch_policy, source_tensor_dtype_profile?`. `LensForgeFile { role, path, sha256, bytes }`. `source_tensor_dtype_profile` is `{ format, file_count, tensor_count, element_count, dtypes: [{ dtype, tensor_count, element_count }], fingerprint_sha256 }`. Candle/Qwen require an explicit `cpu` or `cuda:<ordinal>` execution device and the source profile; other runtimes reject both fields until their runtime contracts consume them. Missing legacy device/profile bytes are migration errors, never defaulted or inferred.

`lens_spec_from_manifest` (and `_path`, `_with_license_override`): validate fields and runtime execution compatibility, check license allowance, verify every file against its SHA-256, recompute the Candle/Qwen source profile from those verified physical bytes, derive the artifact weight hash and runtime-specific frozen contract, and return a `LensSpec`. A malformed profile is configuration-invalid; a structurally valid profile that disagrees with the verified bytes is a frozen violation. Candle/Qwen identity includes exact device, declared model dtype, and source-profile fingerprint while the stored slot dtype remains F32. `register_commissioned` registers the result.
...
| TEI timeout / max_batch | 30 s / 64 | `TeiHttpLens` |
| `DEFAULT_TEI_ENDPOINT` | `http://127.0.0.1:18190/embed` | `tei_http.rs` |
