...
The numeric runtime. A single `Backend` trait (gemm, cosine, dot, l2, normalize, topk) implemented twice — CPU SIMD over `wide` f32x8/f32x16 and a CUDA `sm_120` backend — engineered for bit-near parity (deterministic nvcc flags, matched reduction order, 1e-3 rel / 1e-6 abs golden tolerances). f32 GEMM uses cuBLAS; distance/topk use embedded PTX. Quantization includes TQPR-v2 TurboQuant: deterministic implicit Haar rotation, sphere-coordinate Lloyd–Max scalar codebooks, alternating mixed widths at exact 2.5/3.5 logical data-bit budgets for even dimensions (with the wider final scalar code reported explicitly for odd dimensions), and a one-bit Gaussian-QJL residual scored asymmetrically from an uncompressed prepared query directly against packed candidates. Its authenticated header and complete geometry identity make format, padding, length, and codec drift fail closed. The runtime also supplies a 1-bit binary codec with Hamming prefilter and separate MXFP4/MXFP8 codecs. Grouped/ragged GEMM and a persisted JSON autotune cache with epsilon-greedy/Thompson exploration and a >2%-margin A/B promotion gate complete the layer. Errors are typed `ForgeError`s rather than silent fallback.

### 3.4 Lens registry — `calyx-registry` (S3) → [07_registry_lenses.md](07_registry_lenses.md)
The embedder/lens registry. Defines the `Lens` trait and seven deterministic runtimes (algorithmic, TEI HTTP, Candle local BERT, ONNX, static-lookup mmap, external-command subprocess, multimodal adapter). Every lens is frozen behind a content-addressed `FrozenLensContract`/`LensId` (length-delimited BLAKE3 over name, weight hash, corpus hash, shape fingerprint). The `Registry` fails closed, validating shape, modality, and norm policy per measurement. Provides hot-swap, priority backfill, CPU/GPU placement budgeting, ingest microbatch admission with circuit breakers, capability cards + signal/correlation admit-park-retire gate, eight default domain panels, three algorithmic temporal lenses, matryoshka compression, and DB-native lens catalog persistence; legacy JSON registry files are migration input only.
