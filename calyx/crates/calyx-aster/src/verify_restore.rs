...
//!
//! Adopted into ASTROLABE's owned base from the calyx-aster vault-lifecycle
//! module (upstream `07a3a953`, "add vault lifecycle RPCs"). The upstream module
//! threaded an optional `SharedVaultContext` for value-at-rest crypto; our owned
//! base has no value-crypto feature, so the crypto plumbing is intentionally not
//! adopted here. Adopting value-at-rest encryption is a separate feature (its own
//! `security::value_crypto` module upstream) and is out of scope for the
//! verify-restore durability adoption (Refs #296).
