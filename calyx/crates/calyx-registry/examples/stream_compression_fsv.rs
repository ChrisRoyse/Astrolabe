//! Full State Verification driver for #563 and #574 (wave-26, lane w26-tq).
//!
...
use calyx_core::{
    Asymmetry, Modality, QuantPolicy, Slot, SlotId, SlotResource, SlotShape, SlotState,
    SlotVector, SystemClock, VaultId, VaultStore,
};
use calyx_ledger::EntryKind;
use calyx_registry::{
    AlgorithmicLens, COMPRESSED_SLOT_TAG, COMPRESSION_GENERATION_MARKER, CompressionQuery,
    LensRuntime, LensSpec, Registry, StoredSlotCodec,
};
use serde_json::json;
...
fn main() {
    if let Err(error) = run() {
        println!("{}", json!({ "event": "fsv_failure", "error": error.to_string() }));
        std::process::exit(1);
    }
}
...
fn run() -> AnyResult<()> {
    ...
    let root = workspace.join("target").join("w26-tq563-574-fsv");
    if root.exists() {
    ...
    let tq35 = register(&mut registry, "w26-stream-tq35", 41, QuantPolicy::TurboQuant { bits_per_channel_x2: 7 })?;
    let int8 = register(&mut registry, "w26-scalar-int8", 42, QuantPolicy::ScalarInt8)?;
    let tq16 = register(&mut registry, "w26-tq16-refused", 43, QuantPolicy::TurboQuant { bits_per_channel_x2: 16 })?;
    let empty_slot = register(&mut registry, "w26-empty-column", 44, QuantPolicy::TurboQuant { bits_per_channel_x2: 5 })?;
    ...
    require(
        ...
        format!("QuantPolicy::ScalarInt8 serialized identity must be \"scalar_int8\", got {scalar_json}"),
    )?;
    let parsed_back: QuantPolicy = serde_json::from_str(&scalar_json)?;
    require(parsed_back == QuantPolicy::ScalarInt8, "scalar_int8 must round-trip")?;
    let tq16_json = serde_json::to_string(&QuantPolicy::TurboQuant { bits_per_channel_x2: 16 })?;
    println!(
        "{}",
        json!({
    ...
    for index in 0..ROWS {
        ingester.send(event(index, &[41, 42, 43]), EpochSecs(2_000 + index as i64))?;
    }
    let stats = ingester.drain_and_close()?;
    vault.flush()?;
    require(stats.ingested == ROWS, format!("streamed {ROWS} events, ingested {}", stats.ingested))?;
    ...
    for index in 0..ROWS {
        ...
        require(
            !constellation.metadata.contains_key("quantized")
                && constellation.metadata.keys().all(|key| !key.starts_with("quant_slot_")),
    ...
    require(snapshot > seq_before, "generation write must advance the vault seq")?;

    // Independent persisted-state readback.
    let stored_rows = vault.scan_cf_at(snapshot, ColumnFamily::slot(SlotId::new(41)))?;
    require(stored_rows.len() == ROWS, format!("expected {ROWS} envelope rows, got {}", stored_rows.len()))?;
    for (key, value) in &stored_rows {
        require(value.first().copied() == Some(COMPRESSED_SLOT_TAG), "envelope tag missing")?;
        require(value.get(1).copied() == Some(3), "envelope version must be 3")?;
        require(value.get(2).copied() == Some(1), "envelope codec byte must be 1 (TurboQuantBits3p5)")?;
    ...
    for (index, cx_id) in cx_ids.iter().enumerate() {
        let raw = raw_rows.iter().find(|(key, _)| key == &slot_key(*cx_id)).ok_or("raw sidecar missing")?;
        let expected = encode::encode_slot_vector(&dense_vector(index))?;
        require(raw.1 == expected, format!("row {index}: raw sidecar must be byte-exact source vector"))?;
    }
    let manifest = vault
        .read_cf_at(snapshot, ColumnFamily::Compression, &compression_manifest_key(SlotId::new(41)))?
    ...
    require(entry.kind == EntryKind::Migrate, format!("ledger kind must be Migrate, got {:?}", entry.kind))?;
    require(entry.entry_hash == ledger_ref.hash, "ledger entry hash must equal report.ledger.hash")?;
    let payload_text = String::from_utf8_lossy(&entry.payload).to_string();
    require(
        payload_text.contains(COMPRESSION_GENERATION_MARKER) && payload_text.contains("generation_root_sha256"),
    ...
    require(report_int8.requested_quant == QuantPolicy::ScalarInt8, "requested policy identity")?;
    require(report_int8.stored_codec == StoredSlotCodec::ScalarInt8, "stored codec identity")?;
    ...
    for (_, value) in &int8_rows {
        require(value.get(2).copied() == Some(3), "envelope codec byte must be 3 (ScalarInt8)")?;
        require(value.get(3).copied() == Some(1), "envelope level byte must be 1 (Bits8)")?;
    }
    println!(
        "{}",
        json!({
    ...
    require(seq_before_tq16 == seq_after_tq16, "TQ16 refusal must not advance the vault seq")?;
    require(tq16_manifest.is_none(), "TQ16 refusal must not write a manifest")?;
    require(tq16_first_row != Some(COMPRESSED_SLOT_TAG), "TQ16 rows must remain raw")?;
    require(
        tq16_error.message.contains("removed policy substitution")
            && tq16_error.message.contains("QuantPolicy::ScalarInt8"),
        format!("TQ16 error must name the removed substitution: {}", tq16_error.message),
    )?;
    println!(
        "{}",
        json!({
    ...
    require(
        ...
        format!("legacy TQ16 refusal must carry remediation: {}", legacy_error.message),
    )?;
    println!(
        "{}",
        json!({
    ...
    require(
        dup_error.message.contains("already carries a compressed envelope"),
        format!("duplicate compression refusal: {}", dup_error.message),
    )?;
    require(vault.latest_seq() == seq_before_dup, "duplicate refusal must not write")?;
    ...
    require(hits[0].cx_id == cx_ids[0], format!("top-1 must be row 0 ({}), got {}", cx_ids[0], hits[0].cx_id))?;
    ...
    require(parity > 0.8, format!("decode parity cosine too low: {parity}"))?;
    ...
    let SlotVector::Dense { data: int8_data, .. } = int8_index.read_at(cx_ids[0], head)? else {
        return Err("decoded int8 row must be dense".into());
    };
    let int8_parity = cosine(&raw, &int8_data);
    require(int8_parity > 0.999, format!("int8 decode parity too low: {int8_parity}"))?;
    ...
    drop(reopened);
    fs::remove_dir_all(&root)?;
    println!("{}", json!({ "event": "fsv_success", "issues": [563, 574] }));
    Ok(())
}
...
fn register(
    ...
    let spec = LensSpec {
        ...
        runtime: LensRuntime::Algorithmic { kind: format!("one_hot:{DIM}") },
...
fn open_vault(dir: &Path) -> AnyResult<Arc<AsterVault<SystemClock>>> {
    ...
    Ok(Arc::new(AsterVault::open(dir, vault_id, VAULT_SALT.to_vec(), options)?))
}
...
fn dense_vector(index: usize) -> SlotVector {
    SlotVector::Dense { dim: DIM, data: dense_data(index) }
}
...
fn cosine(left: &[f32], right: &[f32]) -> f64 {
    let dot: f64 = left.iter().zip(right).map(|(a, b)| f64::from(*a) * f64::from(*b)).sum();
    let ln: f64 = left.iter().map(|v| f64::from(*v) * f64::from(*v)).sum::<f64>().sqrt();
    let rn: f64 = right.iter().map(|v| f64::from(*v) * f64::from(*v)).sum::<f64>().sqrt();
    if ln == 0.0 || rn == 0.0 { 0.0 } else { dot / (ln * rn) }
}
