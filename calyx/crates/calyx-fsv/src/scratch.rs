...
mod tests {
    ...
    #[test]
    fn armed_scratch_is_created_then_removed_on_drop() {
        let path;
        {
            let scratch = ScratchDir::new_temp("calyx-fsv-scratch-drop").unwrap();
            path = scratch.path().to_path_buf();
            assert!(path.is_dir(), "scratch dir must exist while guard is live");
            std::fs::write(path.join("payload.bin"), b"data").unwrap();
        }
        // FSV: independent filesystem readback after the guard dropped.
        assert!(
            !path.exists(),
            "armed scratch leaked after drop: {}",
            path.display()
        );
    }

    #[test]
    fn armed_scratch_is_removed_on_panic_unwind() {
        // The defect this issue closes: a panicking test must not leak its scratch.
        let probe = std::sync::Arc::new(std::sync::Mutex::new(PathBuf::new()));
        let probe_inner = probe.clone();
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            let scratch = ScratchDir::new_temp("calyx-fsv-scratch-panic").unwrap();
            *probe_inner.lock().unwrap() = scratch.path().to_path_buf();
            // Non-empty: prove recursive removal, not just an empty-dir rmdir.
            std::fs::create_dir_all(scratch.path().join("nested")).unwrap();
            std::fs::write(scratch.path().join("nested/evidence.bin"), b"x").unwrap();
            assert!(scratch.path().is_dir());
            panic!("intentional unwind through a live ScratchDir");
        }));
        ...
        // FSV: the directory must be GONE after the unwinding test.
        assert!(
            !leaked.exists(),
            "scratch dir leaked after panic unwind: {}",
            leaked.display()
        );
    }

    #[test]
    fn kept_scratch_survives_drop() {
        let root = ScratchDir::new_temp("calyx-fsv-scratch-keep-outer").unwrap();
        let kept_path = root.path().join("kept-root");
        {
            let kept = ScratchDir::kept(kept_path.clone());
            assert!(kept.is_kept());
            assert!(kept.path().is_dir());
            std::fs::write(kept.path().join("evidence.bin"), b"keep-me").unwrap();
        }
        // FSV: a kept (operator-root) guard must NOT delete on drop.
        assert!(
            kept_path.join("evidence.bin").exists(),
            "kept scratch was wrongly removed: {}",
            kept_path.display()
        );
        // outer `root` (armed) cleans the whole subtree at end of scope.
    }

    #[test]
    fn into_kept_disarms_cleanup() {
        let path;
        {
            let scratch = ScratchDir::new_temp("calyx-fsv-scratch-into-kept").unwrap();
            path = scratch.into_kept();
        }
        assert!(
            path.is_dir(),
            "into_kept must disarm removal: {}",
            path.display()
        );
        // Manual cleanup now that the guard no longer owns it.
        std::fs::remove_dir_all(&path).unwrap();
    }

    #[test]
    fn scratch_or_temp_falls_back_armed_when_unset() {
        let guard = scratch_or_temp("CALYX_SCRATCH_TEST_UNSET_KEY", "calyx-fsv-scratch-or-temp");
        assert!(
            !guard.is_kept(),
            "unset variable must yield an armed fallback"
        );
        assert!(guard.path().is_dir());
        let path = guard.path().to_path_buf();
        drop(guard);
        assert!(!path.exists(), "fallback scratch leaked after drop");
    }

    #[test]
    fn deref_and_asref_behave_like_a_path() {
        let scratch = ScratchDir::new_temp("calyx-fsv-scratch-deref").unwrap();
        // Deref to Path: method resolution and join work through the guard.
        let child = scratch.join("child.txt");
        assert!(child.ends_with("child.txt"));
        // AsRef<Path>: usable anywhere `impl AsRef<Path>` is accepted.
        fn takes_path(p: impl AsRef<Path>) -> bool {
            p.as_ref().is_dir()
        }
        assert!(takes_path(&scratch));
    }
}
