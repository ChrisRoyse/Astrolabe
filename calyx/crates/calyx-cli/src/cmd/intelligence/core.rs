...
use calyx_core::{
    AnchorKind, AnchorValue, Constellation, CxId, Panel, Slot, SlotId, SlotState, SlotVector,
};
...
) -> CliResult<std::collections::BTreeMap<CxId, Constellation>> {
    Ok(calyx_search::load_docs_resolved(
        &context.vault,
        &context.state,
    )?)
}
