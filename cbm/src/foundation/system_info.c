...
int cbm_default_worker_count(bool initial) {
    /* CBM_WORKERS env override (strict integer in [1, CBM_WORKERS_MAX]).
    ...
    char buf[CBM_SZ_32];
    int env_status = cbm_read_env("CBM_WORKERS", buf, sizeof(buf));
    if (env_status < 0) {
        cbm_log_error("workers.env.invalid", "code", "CBM_WORKERS_UNREADABLE", "name",
                      "CBM_WORKERS", "message",
                      "CBM_WORKERS is present but could not be read completely", "remediation",
    ...
    if (env_status > 0) {
        ...
        if (errno == 0 && end && *end == '\0' && end != buf && n >= MIN_WORKERS &&
            n <= CBM_WORKERS_MAX) {
            return (int)n;
        }
        cbm_log_error("workers.env.invalid", "code", "CBM_WORKERS_INVALID", "name",
                      "CBM_WORKERS", "value", buf, "min", "1", "max", "256", "message",
                      "CBM_WORKERS must be an exact decimal integer from 1 through 256",
                      "remediation",
                      "set CBM_WORKERS to a valid worker count or remove it to use auto-detection");
        return CBM_NOT_FOUND;
    }
