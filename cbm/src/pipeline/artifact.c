...
static int write_metadata(const char *repo_path, const char *project_name, int64_t nodes,
                          ...
                          int compression_level, const char *compressed_sha256,
                          const char *database_sha256) {
...
/* Reachable only from the Windows snapshot/import path; guarded to match its
 * callers rather than left as dead code on this host. */
#if defined(_WIN32)
...
static char *prepare_export_snapshot(const char *db_path, const char *project_name, int quality,
                                     size_t *out_size, int64_t *out_nodes, int64_t *out_edges) {
#ifndef _WIN32
    (void)db_path;
    (void)project_name;
    (void)quality;
...
int cbm_artifact_export(const char *db_path, const char *repo_path, const char *project_name,
    ...
    size_t db_size = 0;
    /* 64-bit (#952). Filled either by prepare_export_snapshot (Windows-only
     * path) or by the direct count below on this host; one declaration serves
    ...
    if (count_store) {
        ...
        cbm_store_close_required(&count_store, "artifact.export.count_store");
    }
