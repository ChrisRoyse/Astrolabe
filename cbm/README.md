...
| `CBM_LOG_FORMAT` | `text` | Select exact case-insensitive `text` or newline-delimited `json` for libcbm diagnostics. An absent variable selects `text`. A present empty, whitespace-only, or otherwise unrecognized value refuses startup with `CBM_LOG_FORMAT_INVALID`, the exact value byte length and hex, and remediation before any tool or store initialization. JSON mode preserves one complete structured object per logger record. |
| `ASTRO_CBM_PIPELINE_PHASE_TRACE` | `false` | Set to exact `1`, `true`, or `TRUE` to emit start/end phase telemetry. This explicit measurement channel is independent of `CBM_LOG_LEVEL`; it uses the legacy `ASTRO_CBM_PIPELINE_PHASE_TRACE key=value` record in text mode and a typed `pipeline.phase_trace` object in JSON mode. |
