...
| `CBM_LOG_FORMAT` | `text` | Select `text` or newline-delimited `json` for libcbm diagnostics. Selection is case-insensitive and process-local. Unset, empty, or invalid values leave the current selection unchanged; a fresh process therefore remains `text`. JSON mode preserves one complete structured object per logger record. |
