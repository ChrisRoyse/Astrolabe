        ...
        if (d->label && (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0)) {
            ...
            if (strcmp(d->label, "Method") == 0 && d->receiver && d->receiver[0]) {
                ...
                if (end > r) {
                    ...
                    const CBMRegisteredType *existing =
                        cbm_registry_lookup_type(&reg, rf.receiver_type);
                    if (!existing) {
                        ...
                        if (!ts_node_is_null(result_node)) {
                            ...
                            if (strcmp(ts_node_type(result_node), "parameter_list") == 0) {
                                ...
                                for (uint32_t ri = 0; ri < rnc && idx < ret_count; ri++) {
                                    ...
                                    if (ts_node_is_null(rtype))
                                        rtype = rchild;
                                    new_rets[idx++] = parse_type_node_with_params(
                                        arena, rtype, source, module_qn, tp_names);
                                }
                                new_rets[idx] = NULL;
                            } else {
                                new_rets[0] = parse_type_node_with_params(
                    ...
                    if (!ts_node_is_null(params_node)) {
                        ...
                        for (uint32_t pi = 0; pi < pnc && idx < pc; pi++) {
                            ...
                            if (ts_node_is_null(pchild) || !ts_node_is_named(pchild))
                                continue;
                            if (strcmp(ts_node_type(pchild), "parameter_declaration") != 0)
                                continue;
                            TSNode ptype = ts_node_child_by_field_name(pchild, "type", 4);
                            if (ts_node_is_null(ptype))
                                continue;
                            new_params[idx++] = parse_type_node_with_params(arena, ptype, source,
                                                                            module_qn, tp_names);
                        }
                        new_params[idx] = NULL;
                        if (idx > 0) {
                            ...
                            if (sig != reg->funcs[fi].signature) {
                                ...
                                ((CBMType *)reg->funcs[fi].signature)->data.func.param_types =
                                    new_params;
                            } else {
                                CBMType *new_sig =
                                    (CBMType *)cbm_arena_alloc(arena, sizeof(CBMType));
                                *new_sig = *sig;
...
int cbm_go_fast_resolve_qualified_calls(CBMFileResult *result, CBMTypeRegistry *reg,
    ...
    for (int i = 0; i < initial_count; i++) {
        ...
        rc.confidence = 0.92f;
        rc.reason = NULL;
        if (!cbm_resolvedcall_push(&result->resolved_calls, &result->arena, rc)) {
            return 0;
        }
        newly_resolved++;
    }
