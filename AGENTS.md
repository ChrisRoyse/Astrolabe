...
- WSL is not part of this development environment. Do not install, start, invoke, or retain WSL services, packages, install roots, registered distributions, or processes for this project. The native launcher must fail closed before bootstrap, build, or test work if any WSL installation state remains, a WSL process is active, or any running `bash.exe` cannot be verified under `C:\Program Files\Git\`. Detecting WSL during a batch invalidates all evidence and requires immediate interruption, launcher-owned process cleanup, and verified `target/` removal. Before handoff, verify `WSLService`, `LxssManager`, WSL package/install-root/distribution state, `wsl`, `wslhost`, and `vmmemWSL` are absent; when WSL removal is explicitly requested, use native Windows management tools to stop, disable, or uninstall it and record any permission or host-level blocker in GitHub.
