...
(`SKIP[ASTRO_CBM_INCREMENTAL_LINUX_REQUIRED]`; that coverage is owned by the
required Linux CI jobs `cbm tests / linux-x64-gcc` and `linux-x64-clang`).
...
| System | CI label | Expected registered tests | Basis |
|---|---|---:|---|
